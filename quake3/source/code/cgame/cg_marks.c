...
//PKMOD - Ergodic 03/18/04 - change max particles from 1024 to 1536
#define		MAX_PARTICLES	1536
...
//PKMOD - Ergodic 01/18/02 - re-sized sparks - now even bigger!
void CG_ParticleSparks2 (vec3_t org, vec3_t vel, int duration, float x, float y, float speed)
{
	...
	p->pshader = cgs.media.pkagravitywellspark;
...
void CG_ParticleSparks3 (vec3_t org, vec3_t vel, int duration, float x, float y, float speed)
{
	...
	//PKMOD - Ergodic 09/22/03 - change color
	p->color = EMISIVEFADE;
//	p->color = GREY75;
	p->alpha = 0.4f;
	p->alphavel = 0;

	//PKMOD - Ergodic 10/15/03 - vary the size
	ransize = 2.0 + 1.25 * random();
	p->height = ransize;
	p->width = ransize;
	p->endheight = 2.0;
	p->endwidth = 2.0;

	switch ( rand() % 5 ) {	//random numbers: { 0, 1, 2, 3, 4 }
		case 0:
			p->pshader = cgs.media.pkabeartrapspark1Shader;
			break;
		case 1:
			p->pshader = cgs.media.pkabeartrapspark2Shader;
			break;
		case 2:
			p->pshader = cgs.media.pkabeartrapspark3Shader;
			break;
		case 3:
			p->pshader = cgs.media.pkabeartrapspark4Shader;
			break;
		default:
			p->pshader = cgs.media.pkabeartrapspark5Shader;
			break;
		}

	//PKMOD - Ergodic 09/22/03 - change type
	//p->type = P_SMOKE;
	p->type = P_SMOKE_IMPACT;
