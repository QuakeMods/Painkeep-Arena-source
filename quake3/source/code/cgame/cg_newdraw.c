...
//PKMOD - Ergodic 02/08/04 - get maximum weapon ammo
//	NOTES - these value should be in sync with void Add_Ammo in g_items.c

int CG_GetMaxAmmo( int weapon) {
	switch( weapon ) {
		case WP_GAUNTLET:		//weapon 1
			return (-1);
		break;
		case WP_MACHINEGUN:		//weapon 2
			return (200);
		break;
		case WP_SHOTGUN:		//weapon 3
			return (200);
		break;
		case WP_AIRFIST :		//weapon 4
			return (4);
		break;
		case WP_NAILGUN :		//weapon 5
			return (200);
		break;
		case WP_GRENADE_LAUNCHER:	//weapon 6
			return (200);
		break;
		case WP_ROCKET_LAUNCHER:	//weapon 7
			return (200);
		break;
		case WP_LIGHTNING:		//weapon 8
			return (200);
		break;
		case WP_RAILGUN:		//weapon 9
			return (200);
		break;
		case WP_GRAPPLING_HOOK:		//weapon 10
			return (-1);
		break;
		case WP_GRAVITY:		//weapon 11
			return (1);
		break;
		case WP_SENTRY:			//weapon 12
			return (3);
		break;
		case WP_BEARTRAP:		//weapon 13
			return (3);
		break;
		case WP_BEANS:			//weapon 14
			return (1);
		break;
		case WP_EXPLODING_SHELLS:	//weapon 15
			return (10);
		break;
...
//PKMOD - ergodic 02/07/04 - add horizontal meters for ammo 
//Meter: Horizontal Right to Left fill
static void CG_DrawPlayerAmmoMeter_HR2L(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	float	value;
	float	rectx;
	float	rectw;
	int		max_value;
	...
	if ( cent->currentState.weapon ) {
		max_value = CG_GetMaxAmmo( cent->currentState.weapon );
		if ( max_value < 0 )
			value = 1;
		else {
			//force a float value
		...
		if (shader) {
			rectx = rect->x + rect->w * ( 1 - value );
			rectw = rect->w * value;
		    trap_R_SetColor( color );
			CG_DrawPic(rectx, rect->y, rectw, rect->h, shader);
			trap_R_SetColor( NULL );
		}
	}
}
...
//PKMOD - ergodic 02/07/04 - add horizontal meters for ammo 
//Meter: Horizontal Left to Right fill
static void CG_DrawPlayerAmmoMeter_HL2R(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	float	value;
	float	rectw;
	int		max_value;
	centity_t	*cent;
	...
	if ( cent->currentState.weapon ) {
		...
		if (shader) {
			rectw = rect->w * value;
		    trap_R_SetColor( color );
			CG_DrawPic(rect->x, rect->y, rectw, rect->h, shader);
			trap_R_SetColor( NULL );
		}
	}
}

//PKMOD - ergodic 02/08/04 - add horizontal meters for armor 
//Meter: Horizontal Right to Left fill
static void CG_DrawPlayerArmorMeter_HR2L(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	float	value;
	float	rectx;
	float	rectw;
	centity_t	*cent;
...
//PKMOD - ergodic 02/07/04 - add horizontal meters for armor 
//Meter: Horizontal Left to Right fill
static void CG_DrawPlayerArmorMeter_HL2R(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	float	value;
	float	rectw;
	centity_t	*cent;
	playerState_t	*ps;
...
//PKMOD - Ergodic 02/08/04 - add relative score value to the HUD's gauges 
static void CG_DrawPlayerRelativeScore( rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	char num[16];
	int value;


	if ( cg.snap->ps.persistant[PERS_SCORE] == cgs.scores1 ) {
		//PKMOD - Ergodic 02/09/04 - debug wackie scores when respawn into game (inactive)
		//Com_Printf( "RelativeScore - if " );
		//check if there is a scond place score...
		if ( cgs.scores2 == SCORE_NOT_PRESENT )
			value = cg.snap->ps.persistant[PERS_SCORE];
		else
			value = cg.snap->ps.persistant[PERS_SCORE] - cgs.scores2;
	} else {
		//PKMOD - Ergodic 02/09/04 - debug wackie scores when respawn into game (inactive)
		//Com_Printf( "RelativeScore - else " ); 
		value = cg.snap->ps.persistant[PERS_SCORE] - cgs.scores1;
	}

	//PKMOD - Ergodic 02/09/04 - debug wackie scores when respawn into game (inactive)
	//Com_Printf( "- pers_score>%d<, score1>%d<, score2>%d<, value>%d<\n", cg.snap->ps.persistant[PERS_SCORE], cgs.scores1, cgs.scores2, value); 
...
static void CG_DrawPlayerItem( rectDef_t *rect, float scale, qboolean draw2D) {
	//int		value;	//PKMOD - Ergodic 02/01/04 - remove original code
	//vec3_t	origin, angles;	//PKMOD - Ergodic 02/01/04 - remove original code
	int		i;
	int		holdable_index;
	int		y;
...
//	CG_RegisterItemVisuals( value );
	...
	//MOdify the code to handle more than one holdable in possession
	if ( cg.snap->ps.stats[STAT_HOLDABLE_ITEM] ) {
		//PKMOD - Ergodic 10/13/01 - move holdable icons higher up on the screen
		//			change start height from 1/2 to 1/4 from top of screen
		//		y = (SCREEN_HEIGHT-ICON_SIZE_SMALL)/2;
		y = (SCREEN_HEIGHT-ICON_SIZE_SMALL)/4;
		holdable_index = cg.snap->ps.stats[STAT_ACTIVE_HOLDABLE];
		for (i = 1; i < HI_NUM_HOLDABLE; i++) {
			if ( cg.snap->ps.stats[STAT_HOLDABLE_ITEM] & ( 1 << holdable_index ) ) {
				//PKMOD - Ergodic 12/07/01 - display Private Bot pieces as only one Icon
				if ( ( holdable_index < HI_BOTLEGS ) || ( holdable_index > HI_BOTHEAD )) {
					//only display valid holdables
					if ( holdable_index < HI_NUM_HOLDABLE ) {
						CG_RegisterItemVisuals( cg_holdable[holdable_index] );
						CG_DrawPic( 640-ICON_SIZE, y, ICON_SIZE_SMALL, ICON_SIZE_SMALL, cg_items[ cg_holdable[holdable_index] ].icon );
					}
					//get next y offset
					y += 1.2 * ICON_SIZE_SMALL;
				}
			}
			//PKMOD - Ergodic 12/07/01 - now at end of Private Bot pieces list - then display
			if ( holdable_index == HI_BOTHEAD ) {
	//PKMOD - Ergodic 12/07/01 - hold private bot parts
				int			pribot_parts;
				qhandle_t	pribotIcon;

				pribot_parts = cg.snap->ps.stats[STAT_HOLDABLE_ITEM] & ( 7 << HI_BOTLEGS );
				if ( pribot_parts ) { 
					//encoding bits (LEGS TORSO HEAD)
					switch ( pribot_parts >> HI_BOTLEGS ) {
						case 1:
							pribotIcon = cgs.media.pkapribot_100Icon;
							break;
						case 2:
							pribotIcon = cgs.media.pkapribot_010Icon;
							break;
						case 3:
							pribotIcon = cgs.media.pkapribot_110Icon;
							break;
						case 4:
							pribotIcon = cgs.media.pkapribot_001Icon;
							break;
						case 5:
							pribotIcon = cgs.media.pkapribot_101Icon;
							break;
						case 6:
							pribotIcon = cgs.media.pkapribot_011Icon;
							break;
						case 7:
							pribotIcon = cgs.media.pkapribot_111Icon;
							break;
					}
					CG_DrawPic( 640-ICON_SIZE, y, ICON_SIZE_SMALL, ICON_SIZE_SMALL, pribotIcon );
					//get next y offset
					y += 1.2 * ICON_SIZE_SMALL;
				}
			}
...
//>>>>>>
/* PKMOD - Ergodic 02/01/04 - remove original code
   value = cg.snap->ps.stats[STAT_HOLDABLE_ITEM];
	if ( value ) {
		CG_RegisterItemVisuals( value );
...
//PKMOD - Ergodic 01/28/04 - Dynamic HUD activation: Ignore this code since PKA does not have TA type of powerups
/*
...
static void CG_DrawAreaPowerUp(rectDef_t *rect, int align, float special, float scale, vec4_t color) {
	...
	for ( i = 0 ; i < active ; i++ ) {
		...
		//PKMOD - Ergodic - 02/25/04 - add cases for HOLDABLE POWERUPs
		if ( sorted[i] == PW_RADIATE ) {
			item = BG_FindItemForHoldable( HI_RADIATE );
		} else if ( sorted[i] == PW_PERSENTRY ) {
			item = BG_FindItemForHoldable( HI_PERSENTRY );
		} else 
...
float CG_GetValue(int ownerDraw) {
	...
	//PKMOD - ergodic 02/07/04 - for use in meteres
//	int		max_value;
  ...
  case CG_BLUE_SCORE:
	...
	//PKMOD - ergodic 02/07/04 - add horizontal meters for ammo 
	case CG_PLAYER_AMMO_METER_HR2L:	//Meter: Horizontal Right to Left fill
		if ( cent->currentState.weapon ) {
			//PKMOD - ergodic 02/09/04 - turn off addColorRange functionality for several PKA weapons
	...
	break;
	case CG_PLAYER_AMMO_METER_HL2R:	//Meter: Horizontal Left to Right fill
		if ( cent->currentState.weapon ) {
			//PKMOD - ergodic 02/09/04 - turn off addColorRange functionality for several PKA weapons
			switch ( cent->currentState.weapon ) {
				case WP_GAUNTLET:
	...
	//PKMOD - Ergodic 02/08/04 - add relative score value to the HUD's gauges 
	case CG_PLAYER_RELATIVE_SCORE:
		if ( cg.snap->ps.persistant[PERS_SCORE] == cgs.scores1 )
			return ( cg.snap->ps.persistant[PERS_SCORE] - cgs.scores2 );
		else
			return ( cg.snap->ps.persistant[PERS_SCORE] - cgs.scores1 );
    break;


	//PKMOD - ergodic 02/08/04 - add horizontal meters for armor 
	case CG_PLAYER_ARMOR_METER_HR2L:	//Meter: Horizontal Right to Left fill
		return ps->stats[STAT_ARMOR];
	break;
	case CG_PLAYER_ARMOR_METER_HL2R:	//Meter: Horizontal Left to Right fill
		return ps->stats[STAT_ARMOR];
	break;
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vec4_t color, qhandle_t shader, int textStyle) {
  ...
  case CG_PLAYER_AMMO_VALUE:
	...
	//PKMOD - Ergodic 02/07/04 - add horizontal meters for ammo 
  case CG_PLAYER_AMMO_METER_HR2L:	//Meter: Horizontal Right to Left fill
    CG_DrawPlayerAmmoMeter_HR2L(&rect, scale, color, shader, textStyle);
    break;
  case CG_PLAYER_AMMO_METER_HL2R:	//Meter: Horizontal Left to Right fill
    CG_DrawPlayerAmmoMeter_HL2R(&rect, scale, color, shader, textStyle);
    break;

	//PKMOD - Ergodic 02/08/04 - add horizontal meters for armor 
  case CG_PLAYER_ARMOR_METER_HR2L:	//Meter: Horizontal Right to Left fill
    CG_DrawPlayerArmorMeter_HR2L(&rect, scale, color, shader, textStyle);
    break;
  case CG_PLAYER_ARMOR_METER_HL2R:	//Meter: Horizontal Left to Right fill
    CG_DrawPlayerArmorMeter_HL2R(&rect, scale, color, shader, textStyle);
    break;

	//PKMOD - Ergodic 02/08/04 - add relative score value to the HUD's gauges 
  case CG_PLAYER_RELATIVE_SCORE:
    CG_DrawPlayerRelativeScore(&rect, scale, color, shader, textStyle);
    break;
