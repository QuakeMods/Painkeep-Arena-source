...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	...
	if ( !CG_RegisterClientSkin( ci, teamName, modelName, skinName, headName, headSkinName ) ) {
		if ( teamName && *teamName) {
			Com_Printf( "(CG 1) Failed to load skin file: %s : %s : %s, %s : %s\n", teamName, modelName, skinName, headName, headSkinName );
			...
			if ( !CG_RegisterClientSkin( ci, newTeamName, modelName, skinName, headName, headSkinName ) ) {
				Com_Printf( "(CG 2) Failed to load skin file: %s : %s : %s, %s : %s\n", newTeamName, modelName, skinName, headName, headSkinName );
				return qfalse;
			}
		} else {
			Com_Printf( "(cg 3) Failed to load skin file: %s : %s, %s : %s\n", modelName, skinName, headName, headSkinName );
			return qfalse;
		}
	}
...
//	Com_Printf("CG_NewClientInfo - configstring>%s<\n", configstring);
	...
	//PKMOD - Ergodic 01/06/02 - debug bot's userinfo (inactive)
//	Com_Printf("CG_NewClientInfo - privateBot>%s<\n", v );

	if ( atoi( v ) == 1 )
		newInfo.privateBot = qtrue;
	else
		newInfo.privateBot = qfalse;
...
static void CG_PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] ) {
	...
	//PKMOD Ergodic 07/07/01 - debug new time2 encoding scheme (inactive)
//	Com_Printf("CG_PlayerAngles - angles2[YAW]>%f<, time2>%d<\n", cent->currentState.angles2[YAW], ( cent->currentState.time2 >> 7 ) & 7 );
...
//		dir = cent->currentState.angles2[YAW];
		dir = ( cent->currentState.time2 >> 7 ) & 7;
		if ( dir < 0 || dir > 7 ) {
...
static void CG_PlayerPowerups( centity_t *cent, refEntity_t *torso ) {
	...
	//PKMOD - Ergodic 10/18/01 - debug modelindex2 usage for spare bits (inactive)
//	if ( (rand() % 101) > 95 )
//		Com_Printf("CG_PlayerPowerups - modelindex2>%d<\n", cent->currentState.modelindex2);
	...
	//PKMOD - Ergodic 12/05/01 - add radiate waring sound
	if ( powerups & ( 1 << PW_RADIATE ) ) {
		trap_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.pkaradiatewarningSound );
	}

	//PKMOD - Ergodic 08/02/02 - add Personal Sentry hover sound
	if ( powerups & ( 1 << PW_PERSENTRY ) ) {
		trap_S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.pkapersentryhoverSound );
	}
	...
	if ( powerups & ( 1 << PW_REDFLAG ) ) {
		//PKMOD - Ergodic 08/29/01 - if not carrying the red flag
		if ( ( cent->currentState.generic1 & 15 ) != PW_REDFLAG ) { 
			if (ci->newAnims) {
	...
	if ( powerups & ( 1 << PW_BLUEFLAG ) ) {
		//PKMOD - Ergodic 08/29/01 - if not carrying the blue flag
		if ( ( cent->currentState.generic1 & 15 ) != PW_BLUEFLAG ) { 
			if (ci->newAnims){
...
static void CG_PlayerSprites( centity_t *cent ) {
	...
	if ( cent->currentState.eFlags & EF_AWARD_PAINKILLER ) {
		CG_PlayerFloatSprite( cent, cgs.media.medalPainKiller );
		return;
	}
...
void CG_AddRefEntityWithPowerups( refEntity_t *ent, entityState_t *state, int team ) {
	...
	//PKMOD - Ergodic 08/22/00 - add logic for chainlightning player hit effect
	if ( state->powerups & ( 1 << PW_CLGPLAYERHIT ) ) {
		trap_R_AddRefEntityToScene( ent );  //add raw player "piece" image
		ent->customShader = cgs.media.clgplayerhitShader;
		trap_R_AddRefEntityToScene( ent );  //add player "piece" image with shader effect
	} else {
...
void CG_Player( centity_t *cent ) {
	...
	//PKMOD - Ergodic 11/30/01 - debug modelindex2 usage for spare bits (inactive)
//	if ( (rand() % 101) > 95 )
//		Com_Printf("CG_Player - modelindex2>%d<\n", cent->currentState.modelindex2);
	...
	//PKMOD - Ergodic 09/11/02 - display the Private Bot field effect if ent is a Private Bot and visible
	if ( ci->privateBot ) {
		if ( ! (cent->currentState.powerups & ( 1 << PW_INVIS ) ) ) {
			refEntity_t		pbfield;

			//PKMOD - Ergodic 09/11/02 - debug pb field effect (inactive)
//			Com_Printf( "CG_Player - adding PB field\n" );

			memset( &pbfield, 0, sizeof(pbfield) );
			VectorCopy( cent->lerpOrigin, pbfield.origin );
			AnglesToAxis( cent->lerpAngles, pbfield.axis );

			pbfield.hModel = cgs.media.privatebot_CueModel;
			trap_R_AddRefEntityToScene( &pbfield );
		}
	}
...
//PKMOD - Ergodic 11/13/00 - use time2 integer to store beartraps
//PKMOD - Ergodic 07/07/01 - use new packing scheme to encode # of beartraps
	if ( cent->currentState.time2 & 3 )
		CG_BearTraps_Follow( cent );

	//PKMOD - Ergodic 10/24/01 - add logic for radiation effect
	if ( cent->currentState.powerups & ( 1 << PW_RADIATE ) ) {
		if ( cent->PKA_RadiateTime < cg.time ) {
			//PKMOD - Ergodic 11/24/01 - increase spark height
			vec3_t			sparkheight;
			VectorCopy( cent->lerpOrigin, sparkheight );
			sparkheight[2] += DEFAULT_VIEWHEIGHT;

			CG_Radiation( sparkheight );
			cent->PKA_RadiateTime = cg.time + 1000;		//every 1 seconds
		}
	}

	//PKMOD - Ergodic 11/30/01 - add logic for infected radiated player effect
	if ( cent->currentState.modelindex2 & ( 1 << PKA_IRRADIATED ) ) {

		if ( cent->PKA_RadiateInfectTime < cg.time ) {
			//PKMOD - Ergodic 11/24/01 - increase spark height
			vec3_t			sparkheight;
			//PKMOD - Ergodic 12/01/01 - add a random velocity
			vec3_t			vel;

			VectorCopy( cent->lerpOrigin, sparkheight );
			sparkheight[2] += DEFAULT_VIEWHEIGHT + 5 - 2 * ( rand() % 5 );
...
//			CG_RadiationTrail( sparkheight );

			CG_RadiationTrail( sparkheight, vel, 
					  1,		// radius
					  1, 1, 1, 1,	// color
					  500,		// trailTime
					  cg.time,		// startTime
					  ...
					  0,		// flags
					  cgs.media.radiationTrailShader );

//			cent->PKA_RadiateInfectTime = cg.time + 200 + rand() % 250;		//every .2 seconds plus a random bit
			cent->PKA_RadiateInfectTime = cg.time + 100 + rand() % 50;		//every .1 seconds plus a random bit
		}
	}
	...
	CG_AddRefEntityWithPowerups( &head, &cent->currentState, ci->team );
		...
		//PKMOD - Ergodic 05/07/02 - if Personal Sentry is active on player
	if ( cent->currentState.powerups & ( 1 << PW_PERSENTRY ) ) {
//	if ( cent->currentState.modelindex2 & ( 1 << PKA_PERSENTRY_ACTIVE ) ) {

		//PKMOD - Ergodic 05/07/02 - debug persentry model (inactive)
//		Com_Printf( "CG_Player - attaching persentry\n" );

		memcpy(&persentry, &torso, sizeof(torso));
		persentry.hModel = cgs.media.persentry_active;
		holdtime = cg.time / 50;  // sec *  ( 20f/sec / 1000 )
		persentry.frame =  holdtime % 149;
		persentry.oldframe = 0;
		persentry.customSkin = 0;
		trap_R_AddRefEntityToScene( &persentry );

//>>>>>>>>>>>>
/*
		memset( &persentry, 0, sizeof(persentry) );

		persentry.hModel = cgs.media.persentry_active;

//		torso.customSkin = ci->torsoSkin;

		VectorCopy( cent->lerpOrigin, persentry.lightingOrigin );

		CG_PositionRotatedEntityOnTag( &persentry, &legs, ci->legsModel, "tag_persentry");

		persentry.shadowPlane = shadowPlane;
		persentry.renderfx = renderfx;

		CG_AddRefEntityWithPowerups( &persentry, &cent->currentState, ci->team );
