...
static void CG_ParseScores( void ) {
	...
	for ( i = 0 ; i < cg.numScores ; i++ ) {
		//
		//PKMOD - Ergodic 03/16/04 - fix painkiller medal error: change form of index from "i * 14 + n" to "i * 15 + n" 
		...
		cg.scores[i].painkillerCount = atoi(CG_Argv(i * 15 + 18));;
...
//PKMOD - Ergodic 10/13/00 - store hubinfo into cg fields 
/*
=================
CG_ParseHubInfo
...
=================
*/
static void CG_ParseHubInfo( void ) {
	int		i;

	numHubInfoLines = atoi( CG_Argv( 1 ) );

	//PKMOD - Ergodic 10/13/00 - debug inactive
//	Com_Printf("CG_ParseHubInfo: numHubInfoLines >%d<\n", numHubInfoLines );


	//PKMOD - Ergodic 11/01/00 - change format to include a spacer line and timelimit
	for ( i = 0 ; i < numHubInfoLines - 1; i++ ) {
		if ( atoi( CG_Argv( i * 2 + 3 ) ) > 9 ) {
	...
	//PKMOD - Ergodic 10/13/00 - debug inactive
//	Com_Printf("CG_ParseHubInfo: 1 numHubInfoLines[%d]>%s<\n", i, cgs.hubInfoDisplay[i].info );

		strcat( cgs.hubInfoDisplay[i].info, CG_Argv( i * 2 + 3 ) );
	//PKMOD - Ergodic 10/13/00 - debug inactive
//	Com_Printf("CG_ParseHubInfo: 2 numHubInfoLines[%d]>%s<\n", i, cgs.hubInfoDisplay[i].info );

		strcat( cgs.hubInfoDisplay[i].info, "    " );
	//PKMOD - Ergodic 10/13/00 - debug inactive
//	Com_Printf("CG_ParseHubInfo: 3 numHubInfoLines[%d]>%s<\n", i, cgs.hubInfoDisplay[i].info );

		strcat( cgs.hubInfoDisplay[i].info, CG_Argv( i * 2 + 4 ) );
	//PKMOD - Ergodic 10/13/00 - debug inactive
//	Com_Printf("CG_ParseHubInfo: 4 numHubInfoLines[%d]>%s<\n", i, cgs.hubInfoDisplay[i].info );
	}


	//PKMOD - Ergodic 11/01/00 - get the time remaining
	strcpy( cgs.hubInfoDisplay[i].info, "   " );
	strcat( cgs.hubInfoDisplay[i].info, CG_Argv( i * 2 + 3 ) );
	strcat( cgs.hubInfoDisplay[i].info, " " );
	strcat( cgs.hubInfoDisplay[i].info, CG_Argv( i * 2 + 4 ) );
...
void CG_ParseServerinfo( void ) {
	...
	if ( hold_hub_flag ) {
		cgs.fraglimit = atoi( Info_ValueForKey( info, "hub_fraglimit" ) );
		cgs.timelimit = atoi( Info_ValueForKey( info, "hub_timelimit" ) );
	}
	else {
	...
	cgs.maxclients = atoi( Info_ValueForKey( info, "sv_maxclients" ) ) + MAX_PRIVATE_BOTS;
...
//	Com_Printf( "CG_ConfigStringModified - num>%d<, str>%s<\n", num, str );
	
	// do something with it if necessary
	if ( num == CS_MUSIC ) {
		CG_StartMusic();
	//PKMOD - Ergodic 10/14/00 - add alternate music to hub
	} else if ( num == CS_POSTVOTE_MUSIC ) {
		CG_StartPostVoteMusic( str );
...
//PKMOD - Ergodic 01/04/02 - Add bug fix from [TH]DemENtoR that was in the following post:
//			http://www.quake3world.com/ubb/Forum4/HTML/005944.html
...
static void CG_ServerCommand( void ) {
  ...
  if ( Q_stricmp (cmd, "remapShader") == 0 ) {
	...
	//PKMOD - Ergodic 10/13/00 - add hubinfo command for voting	
	if ( !strcmp( cmd, "hubinfo_pka" ) ) {
		CG_ParseHubInfo();
		return;
	}

	//PKMOD - Ergodic 12/10/03 - add hubalternates server command
	//		This code is hit on client/server games
	//			and this code is hit on SP i.e.: /map hub_30 games
	//			and this code is hit on selected SinglePlayer games
	if ( !strcmp( cmd, "cghubalternates" ) ) {
		//PKMOD - Ergodic 03/29/04 - call will now take parameters form the server
		char a1[MAX_HUB_DISPLAY_NAME];
		char a2[MAX_HUB_DISPLAY_NAME];
		char a3[MAX_HUB_DISPLAY_NAME];
		char a4[MAX_HUB_DISPLAY_NAME];

		//PKMOD - Ergodic 03/29/04 - debug cghubalternates call (inactive)
		//Com_Printf( "In CGame CG_ServerCommand - sending console command: hubalternates\n" );


		Q_strncpyz(a1, CG_Argv(1), sizeof(a1));
		Q_strncpyz(a2, CG_Argv(2), sizeof(a2));
		Q_strncpyz(a3, CG_Argv(3), sizeof(a3));
		Q_strncpyz(a4, CG_Argv(4), sizeof(a4));

		//PKMOD - Ergodic 03/29/04 - debug cghubalternates call (inactive)
		//Com_Printf( "In CGame CG_ServerCommand - sending console command: hubalternates\n" );
		//PKMOD - Ergodic 03/29/04 - debug hubalternates info (inactive)
		//Com_Printf("CG_ServerCommand arg1>%s<\n", a1 );
		//Com_Printf("CG_ServerCommand arg2>%s<\n", a2 );
		//Com_Printf("CG_ServerCommand arg3>%s<\n", a3 );
		//Com_Printf("CG_ServerCommand arg4>%s<\n", a4 );

		//PKMOD - Ergodic 03/29/04 - set the ui cvars
		trap_Cvar_Set( "ui_HubAltDisp1", a1 );
		trap_Cvar_Set( "ui_HubAltDisp2", a2 );
		trap_Cvar_Set( "ui_HubAltDisp3", a3 );
		trap_Cvar_Set( "ui_HubAltDisp4", a4 );
