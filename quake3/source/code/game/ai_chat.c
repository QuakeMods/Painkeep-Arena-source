...
PKMOD - Ergodic 12/15/01 - return Private Bot's owner
==================
*/
char *PrivateBotOwner(bot_state_t *bs) {
	int i;
	static char name[32];

	//PKMOD - Ergodic 03/18/02 - is calling BOT a Private Bot?
	if ( g_entities[bs->entitynum].r.svFlags & SVF_PRIVATEBOT ) {
		i = g_entities[bs->entitynum].parent->client->ps.clientNum;
		EasyClientName( i, name, sizeof(name));
	}
	else {
		strcpy( name, "[invalid var]" );
	}
	return name;
}
...
//PKMOD - Ergodic 03/22/01 - change shotgun name to boomstick
		case MOD_SHOTGUN: return "Boomstick";
...
//PKMOD - Ergodic 07/12/01 - change railgun name to magnum
		case MOD_RAILGUN: return "Magnum";
//PKMOD - Ergodic 12/19/03 - change Lightning Gun name to Chain Lightning Gun
		case MOD_LIGHTNING: return "Chain Lightning Gun";
		case MOD_BFG:
		...
		case MOD_REVERSE_LIGHTNING:
			switch ( rand() % 3 ) {
				...
				case 1: 
					...
					break;
				default:
					return "Sizzle Reflector";
					break;
			}
		default:
			//PKMOD - Ergodic 03/28/01 - fix "[unknown weapon]" with a bandaide patch
//			return "[unknown weapon]";
			switch ( rand() % 3 ) {
				case 0:
					return "PainKeep Weapon";
					break;
				case 1: 
					return "Evolved Weapon";
					break;
				default:
					return "Smelly New Weapon";
					break;
			}
...
int BotChat_EnterGame(bot_state_t *bs) {
	...
	BotAI_BotInitialChat(bs, "game_enter",
				...
				"[invalid var]",						// 3
				BotMapTitle(),							// 4
				//PKMOD - Ergodic 12/13/01 - add new variable for Private Bot's Owner
				PrivateBotOwner(bs),					// 5
				NULL);
...
int BotChat_ExitGame(bot_state_t *bs) {
	...
	BotAI_BotInitialChat(bs, "game_exit",
				...
				"[invalid var]",						// 3
				BotMapTitle(),							// 4
				//PKMOD - Ergodic 01/14/02 - add new variable for Private Bot's Owner
				PrivateBotOwner(bs),					// 5
				NULL);
	bs->lastchat_time = FloatTime();
	bs->chatto = CHAT_ALL;
...
int BotChat_StartLevel(bot_state_t *bs) {
	...
	BotAI_BotInitialChat(bs, "level_start",
				...
				"[invalid var]",						// 3
				"[invalid var]",						// 4
				//PKMOD - Ergodic 01/16/02 - add new variable for Private Bot's Owner
				PrivateBotOwner(bs),					// 5
				NULL);
	bs->lastchat_time = FloatTime();
	bs->chatto = CHAT_ALL;
...
int BotChat_Death(bot_state_t *bs) {
	...
	else
	{
		...
		if (bs->botdeathtype == MOD_WATER)
			...
			BotAI_BotInitialChat(bs, "death_telefrag",
				name,									// 0
				"[invalid var]",						// 1
				"[invalid var]",						// 2
				"[invalid var]",						// 3
...
int BotChat_Kill(bot_state_t *bs) {
	...
	else
	{
		...
		if (bs->enemydeathtype == MOD_GAUNTLET) {
			BotAI_BotInitialChat(bs, "kill_gauntlet",
				name,									// 0
				"[invalid var]",						// 1
				"[invalid var]",						// 2
				"[invalid var]",						// 3
			...
			BotAI_BotInitialChat(bs, "kill_rail",
				name,									// 0
				"[invalid var]",						// 1
				"[invalid var]",						// 2
				"[invalid var]",						// 3
			...
			BotAI_BotInitialChat(bs, "kill_telefrag",
				name,									// 0
				"[invalid var]",						// 1
				"[invalid var]",						// 2
				"[invalid var]",						// 3
