...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match) {
	...
	//PKMOD - Ergodic 04/02/02 - check if bot is a Private Bot
	if ( g_entities[bs->entitynum].r.svFlags & SVF_PRIVATEBOT ) {
		//get message originator 
		trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
		//PKMOD - Ergodic 04/02/02 - check if message is from Private bot's owner
		if ( strcmp( bs->teamleader, netname) )
			//don't do anything if command is not coming from the teamleader
			return;
	}
	else {
...
//PKMOD - Ergodic 12/19/03 - change Lightning Gun name to Chain Lightning Gun
		...
		"Red Flag",
		"Blue Flag",
//PKMOD - Ergodic 04/02/02 - add more PKA items
		"Personal Sentry",
		"Radiate",
		"Private Bot (Legs)",
		"Private Bot (Torso)",
