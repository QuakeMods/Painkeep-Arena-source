...
int BotSelectActivateWeapon(bot_state_t *bs) {
	...
	//PKMOD - Ergodic 04/14/01 - remove plasma gun from PKA
...
//		return WEAPONINDEX_PLASMAGUN;
	...
	//PKMOD - Ergodic 04/14/01 - remove BFG from PKA
...
BotClearPath
 ...
 try to deactivate obstacles like proximity mines on the bot's path
  ...
  PKMOD - Ergodic 04/14/01 - change logic to clear beartraps and 
	autosentrys that are on the bot's path
...
//PKMOD - Ergodic 04/14/01 - remove previous kamakazi code 
	...
	if (bs->blockedbyavoidspot_time > FloatTime() &&
		...
		bestdist = 300;
		bestbeartrap = -1;
		for (i = 0; i < bs->numbeartraps; i++) {
			BotAI_GetEntityState(bs->beartraps[i], &state);
		...
		if (bestbeartrap != -1) {
			...
			BotAI_GetEntityState(bs->beartraps[bestbeartrap], &state);
			VectorCopy(state.pos.trBase, target);
			target[2] += 2;
			VectorSubtract(target, bs->eye, dir);
			vectoangles(dir, moveresult->ideal_viewangles);
			// if the bot has a weapon that does splash damage
			//PKMOD - Ergodic 04/14/01 - modify the weapons pick
			if (bs->inventory[INVENTORY_EXPLOSIVESHOTGUN] > 0 && bs->inventory[INVENTORY_EXPLOSIVESHELLS] > 0)
				moveresult->weapon = WEAPONINDEX_EXPLODING_SHELLS;
			else if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 0)
...
int AINode_Seek_ActivateEntity(bot_state_t *bs) {
	...
	//
	bs->tfl = TFL_DEFAULT;
	//PKMOD - Ergodic 02/11/02 - change grapple cvar name from "bot_grapple" to "bot_dragon"
//	if (bot_grapple.integer) bs->tfl |= TFL_GRAPPLEHOOK;
	if (bot_dragon.integer) bs->tfl |= TFL_GRAPPLEHOOK;
...
//	if (bot_grapple.integer) bs->tfl |= TFL_GRAPPLEHOOK;
	...
	//PKMOD - Ergodic 04/15/01 - use beans if needed
	BotEatBeans(bs);
