...
//PKMOD - Ergodic 02/11/02 - add dragon cvar
vmCvar_t bot_dragon;
...
char *ClientName(int client, char *name, int size) {
	...
	if (client < 0 || client >= MAX_CLIENTS) {
		//PKMOD - Ergodic 01/07/02 - modify message to display client for error with /addbot doom
		BotAI_Print(PRT_ERROR, "ClientName: client out of range: Client Number:%d\n", client);
...
int ClientFromName(char *name) {
	...
	if (!maxclients)
		//PKMOD - Ergodic 12/12/01 - include a player buffer for Private_Bots
		maxclients = trap_Cvar_VariableIntegerValue("sv_maxclients") + MAX_PRIVATE_BOTS;
...
==================
BotEatBeans
PKMOD - Ergodic 04/15/01 - bots will select beans at low health times
==================
*/
void BotEatBeans(bot_state_t *bs) {
	//PKMOD - Ergodic 04/15/01 - debug bot name
	...
	if (bs->inventory[INVENTORY_HEALTH] < 70) {
		//PKMOD - Ergodic 04/15/01 - debug beans eating (inactive)
//		Com_Printf("BotEatBeans - health>%d<, beans>%d<\n", bs->inventory[INVENTORY_HEALTH], bs->inventory[INVENTORY_BEANS]);

		if (bs->inventory[INVENTORY_BEANS] > 0) {
			//PKMOD - Ergodic 04/15/01 - debug beans eating (inactive)
//			Com_Printf("BotEatBeans - beans in inventory\n");
			if (bs->weaponnum != WP_BEANS) {

				bs->weaponchange_time = FloatTime();
				bs->weaponnum = WP_BEANS;
				trap_EA_SelectWeapon(bs->client, bs->weaponnum);

				//co-op method for rocket jump (hack) [NOPE - it does not work]
//				bs->tfl |= TFL_ROCKETJUMP;

				//if bot has an enemy then use beans
				if (bs->enemy >= 0) {
					//PKMOD - Ergodic 04/15/01 - debug beans eating (inactive)
//					Com_Printf("BotEatBeans - %s: Eating my beans\n", netname);
...
void BotUpdateInventory(bot_state_t *bs) {
	...
	//PKMOD - Ergodic 09/07/00 add PainKeepArena weapons
	bs->inventory[INVENTORY_GRAVITY] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_GRAVITY)) != 0;
	bs->inventory[INVENTORY_SENTRY] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_SENTRY)) != 0;
	bs->inventory[INVENTORY_TRAP] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_BEARTRAP)) != 0;
	bs->inventory[INVENTORY_BEANS] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_BEANS)) != 0;
	bs->inventory[INVENTORY_AIRFIST] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_AIRFIST)) != 0;
	//PKMOD - Ergodic - 12/17/00 rename NAILGUN to PKA_NAILGUN to resolve 1.27g conflict
	bs->inventory[INVENTORY_PKA_NAILGUN] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_NAILGUN)) != 0;


	//PKMOD - Ergodic 09/07/00 remove plasma gun from PKA
//	bs->inventory[INVENTORY_PLASMAGUN] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_PLASMAGUN)) != 0;
	//PKMOD - Ergodic 09/07/00 remove BFG gun from PKA
//	bs->inventory[INVENTORY_BFG10K] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_BFG)) != 0;
	...
	//PKMOD - Ergodic 04/14/01 remove cells from PKA
//	bs->inventory[INVENTORY_CELLS] = bs->cur_ps.ammo[WP_PLASMAGUN];
	...
	//PKMOD - Ergodic 04/14/01 remove bfg ammo from PKA
//	bs->inventory[INVENTORY_BFGAMMO] = bs->cur_ps.ammo[WP_BFG];

	//PKMOD - Ergodic 09/07/00 add PainKeepArena ammo
	bs->inventory[INVENTORY_GRAVITY_AMMO] = bs->cur_ps.ammo[WP_GRAVITY];
	bs->inventory[INVENTORY_SENTRY_AMMO] = bs->cur_ps.ammo[WP_SENTRY];
	bs->inventory[INVENTORY_TRAP_AMMO] = bs->cur_ps.ammo[WP_BEARTRAP];
	//PKMOD - Ergodic - 12/17/00 rename NAILS to PKA_NAILS to resolve 1.27g conflict
	bs->inventory[INVENTORY_PKA_NAILS] = bs->cur_ps.ammo[WP_NAILGUN];

	//PKMOD - Ergodic - 04/14/01 - add explosive shells - duh
	bs->inventory[INVENTORY_EXPLOSIVESHOTGUN] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_EXPLODING_SHELLS)) != 0;
	bs->inventory[INVENTORY_EXPLOSIVESHELLS] = bs->cur_ps.ammo[WP_EXPLODING_SHELLS];
...
//	bs->inventory[INVENTORY_TELEPORTER] = bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_TELEPORTER;
	bs->inventory[INVENTORY_TELEPORTER] = 0 < ( bs->cur_ps.stats[STAT_HOLDABLE_ITEM] & (1 << HI_TELEPORTER) );
//	bs->inventory[INVENTORY_MEDKIT] = bs->cur_ps.stats[STAT_HOLDABLE_ITEM] == MODELINDEX_MEDKIT;
	bs->inventory[INVENTORY_MEDKIT] = 0 < ( bs->cur_ps.stats[STAT_HOLDABLE_ITEM] & (1 << HI_MEDKIT) );
	//PKMOD - Ergodic 11/10/01 - new holdables
	bs->inventory[INVENTORY_RADIATE] = 0 < ( bs->cur_ps.stats[STAT_HOLDABLE_ITEM] & (1 << HI_RADIATE) );
	//PKMOD - Ergodic 11/23/01 - new holdables
	bs->inventory[INVENTORY_PERSENTRY] = 0 < ( bs->cur_ps.stats[STAT_HOLDABLE_ITEM] & (1 << HI_PERSENTRY) );
	//PKMOD - Ergodic 12/03/01 - New Holdables - Private Bot pieces (Legs, Torso, Head)
	bs->inventory[INVENTORY_PRIBOTLEGS] = 0 < ( bs->cur_ps.stats[STAT_HOLDABLE_ITEM] & (1 << HI_BOTLEGS) );
	bs->inventory[INVENTORY_PRIBOTTORSO] = 0 < ( bs->cur_ps.stats[STAT_HOLDABLE_ITEM] & (1 << HI_BOTTORSO) );
	bs->inventory[INVENTORY_PRIBOTHEAD] = 0 < ( bs->cur_ps.stats[STAT_HOLDABLE_ITEM] & (1 << HI_BOTHEAD) );
	...
	bs->inventory[INVENTORY_BEARTRAPS_ATTACHED] = bs->cur_ps.stats[STAT_BEARTRAPS_ATTACHED];

	//PKMOD - Ergodic 12/20/00 Debug bot state for attached beartraps (inactive)
//	if (bs->inventory[INVENTORY_BEARTRAPS_ATTACHED] > 0) {
//		Com_Printf("BotUpdateInventory - bot has beartrap attached\n" );
//	}
...
void BotBattleUseItems(bot_state_t *bs) {
	//PKMOD - Ergodic 11/01/01 - add radiate holdable logic
	if ( ( bs->inventory[INVENTORY_HEALTH] + bs->inventory[INVENTORY_ARMOR] ) > 75) {
		if (bs->inventory[INVENTORY_RADIATE] > 0) {
			//PKMOD - Ergodic 03/17/04 - set the active holdable item for the BOT 
			g_entities[bs->entitynum].client->ps.stats[STAT_ACTIVE_HOLDABLE] = HI_RADIATE;
			trap_EA_Use(bs->client);
		}
	}

	//PKMOD - Ergodic 11/23/01 - add personal sentry holdable logic
	if (bs->inventory[INVENTORY_PERSENTRY] > 0) {
		//PKMOD - Ergodic 03/17/04 - set the active holdable item for the BOT 
		g_entities[bs->entitynum].client->ps.stats[STAT_ACTIVE_HOLDABLE] = HI_PERSENTRY;
		trap_EA_Use(bs->client);
	}

	//PKMOD - Ergodic 02/12/02 - add private bot holdable logic (must have all the parts)
	if ( ( bs->inventory[INVENTORY_PRIBOTLEGS] + bs->inventory[INVENTORY_PRIBOTTORSO] + bs->inventory[INVENTORY_PRIBOTHEAD] )  == 3 ) {
		//PKMOD - Ergodic 03/17/04 - set the active holdable item for the BOT 
		g_entities[bs->entitynum].client->ps.stats[STAT_ACTIVE_HOLDABLE] = HI_BOTHEAD;
		trap_EA_Use(bs->client);
	}
	...
	if (bs->inventory[INVENTORY_HEALTH] < 40) {
		if (bs->inventory[INVENTORY_TELEPORTER] > 0) {
			if (!BotCTFCarryingFlag(bs)
				...
				//PKMOD - Ergodic 03/17/04 - set the active holdable item for the BOT 
				g_entities[bs->entitynum].client->ps.stats[STAT_ACTIVE_HOLDABLE] = HI_TELEPORTER;
				trap_EA_Use(bs->client);
			}
		}
	}
	if (bs->inventory[INVENTORY_HEALTH] < 60) {
		if (bs->inventory[INVENTORY_MEDKIT] > 0) {
			//PKMOD - Ergodic 03/17/04 - set the active holdable item for the BOT 
			g_entities[bs->entitynum].client->ps.stats[STAT_ACTIVE_HOLDABLE] = HI_MEDKIT;
			trap_EA_Use(bs->client);
		}
	}
#ifdef MISSIONPACK
...
//			bs->inventory[INVENTORY_BFGAMMO] > 7) return 100;
	...
	//PKMOD - Ergodic 09/07/00 bot is aggressive if it has the airfist
	//if the bot can use the airfist
	if (bs->inventory[INVENTORY_AIRFIST] > 0) return 85;

	//PKMOD - Ergodic 09/07/00 remove the usage of the plasmagun
	//if the bot can use the plasmagun
//	if (bs->inventory[INVENTORY_PLASMAGUN] > 0 &&
//			bs->inventory[INVENTORY_CELLS] > 40) return 85;

	//PKMOD - Ergodic 09/07/00 bot is aggressive if it has the nailgun
	//if the bot can use the nailgun
	//PKMOD - Ergodic - 12/17/00 rename NAILGUN to PKA_NAILGUN to resolve 1.27g conflict
	if (bs->inventory[INVENTORY_PKA_NAILGUN] > 0 &&
	//PKMOD - Ergodic - 12/17/00 rename NAIL to PKA_NAILS to resolve 1.27g conflict
			bs->inventory[INVENTORY_PKA_NAILS] > 20) return 75;

	//if the bot can use the grenade launcher
	if (bs->inventory[INVENTORY_GRENADELAUNCHER] > 0 &&
			bs->inventory[INVENTORY_GRENADES] > 10) return 70;

	//PKMOD - Ergodic 09/07/00 bot is aggressive if it has the explosive shells shotgun
	//if the bot can use the explosive shells shotgun
	if (bs->inventory[INVENTORY_SHOTGUN] > 0 &&
			bs->inventory[INVENTORY_EXPLOSIVESHELLS] > 4) return 50;

	//if the bot can use the shotgun
	if (bs->inventory[INVENTORY_SHOTGUN] > 0 &&
			bs->inventory[INVENTORY_SHELLS] > 10) return 40;

	//PKMOD - Ergodic 09/07/00 bot is aggressive if it has a gravity well
	//if the bot can use the gravity well
	if (bs->inventory[INVENTORY_GRAVITY] > 0) return 30;

	//PKMOD - Ergodic 09/07/00 bot is aggressive if it has more than one autosentries
	//if the bot can use the autosentry
	if (bs->inventory[INVENTORY_SENTRY] > 0 && 
			bs->inventory[INVENTORY_SENTRY_AMMO] > 1) return 20;

	//PKMOD - Ergodic 09/07/00 bot is aggressive if it has is closeby and has beartraps (grin)
	if (bs->inventory[INVENTORY_TRAP] > 0 && 
			( bs->inventory[INVENTORY_TRAP_AMMO] > 0 ) && 
			( bs->inventory[ENEMY_HORIZONTAL_DIST] < 200 ) ) return 25;
...
int BotSameTeam(bot_state_t *bs, int entnum) {
	...
	//PKMOD - Ergodic 01/07/02 - Check if Private Bot's owner (don't target PB's owner)
	if ( g_entities[bs->entitynum].r.svFlags & SVF_PRIVATEBOT ) {
		if ( g_entities[bs->entitynum].parent->client->ps.clientNum == entnum ) {
			//PKMOD - Ergodic 01/07/02 - Private Bot's team owner (inactive)
//			Com_Printf("BotSameTeam - Team member detected, entnum>%d<\n", entnum );
			return qtrue;
		}
	}

	//PKMOD - Ergodic 03/18/02 - Check if Private Bot's brother (don't target PB's owned by same parent)
	if ( g_entities[bs->entitynum].r.svFlags & SVF_PRIVATEBOT ) {
		if ( g_entities[entnum].r.svFlags & SVF_PRIVATEBOT ) {
			if ( g_entities[bs->entitynum].parent->client->ps.clientNum == g_entities[entnum].parent->client->ps.clientNum ) {
			//PKMOD - Ergodic 01/07/02 - Private Bot's team owner (inactive)
//			Com_Printf("BotSameTeam - Team member detected, entnum>%d<\n", entnum );
				return qtrue;
			}
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		if (EntityIsInvisible(&entinfo) && !EntityIsShooting(&entinfo)) {
			//PKMOD - Ergodic 10/31/02 - check if enemy has a beartrap attached
			//			attached beartraps will make the player visible to the bots
			entityState_t state;
			BotAI_GetEntityState(i, &state);
			if ( ( state.time2 & 3 ) == 0 )		//if no beartraps are attached
				continue;
		}
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	//PKMOD - Ergodic - 09/07/00 Add PKA weapon specific definitions
	if (wi.number == WP_AIRFIST) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_AIRFIST, 0, 1);
		aim_skill = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_AIRFIST, 0, 1);
	}
	if (wi.number == WP_NAILGUN) {
	...
	if (wi.number == WP_BEARTRAP) {
		aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_BEARTRAP, 0, 1);
		aim_skill = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_BEARTRAP, 0, 1);
	}

	//PKMOD - Ergodic - 09/07/00 Remove plasmagun definition
...
//	}
	//PKMOD - Ergodic - 09/07/00 Remove bfg definition
//	if (wi.number == WP_BFG) {
...
//	}
	...
	if (EntityIsInvisible(&entinfo)) {
		...
		//			attached beartraps will make the player visible to the bots
		entityState_t state;
		BotAI_GetEntityState(bs->enemy, &state);
		if ( ( state.time2 & 3 ) == 0 )		//if no beartraps are attached
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	if (state->eType != ET_MISSILE || state->weapon != WP_GRENADE_LAUNCHER) {
		//PKMOD - Ergodic 03/15/01 - avoid beartraps too
		//PKMOD - Ergodic 04/14/01 - remove avoid beartraps
//		if (state->eType != ET_BEARTRAP || state->weapon != WP_BEARTRAP) {
		//PKMOD - Ergodic 03/15/01 - avoid nails too
		//PKMOD - Ergodic 04/14/01 - remove avoid nails
//			if (state->eType != ET_NAIL || state->weapon != WP_NAILGUN)
		return;
	}
...
/*
==================
BotCheckForBeartraps
//PKMOD - Ergodic 04/14/01 - new code for bots to avoid/target enemy beartraps
==================
*/
void BotCheckForBeartraps(bot_state_t *bs, entityState_t *state) {
	// if this is not a Beartrap
	if (state->eType != ET_BEARTRAP || state->weapon != WP_BEARTRAP)
		return;
	// if this beartrap is from someone on our own team
	...
	if (state->modelindex == BotTeam(bs))
	
		return;
	// if the bot doesn't have a weapon to destroy the beartrap
	//PKMOD - Ergodic - 04/14/01 - add explosive shells - duh
	bs->inventory[INVENTORY_EXPLOSIVESHOTGUN] = (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_EXPLODING_SHELLS)) != 0;
	bs->inventory[INVENTORY_EXPLOSIVESHELLS] = bs->cur_ps.ammo[WP_EXPLODING_SHELLS];

	if (!(bs->inventory[INVENTORY_PKA_NAILGUN] > 0 && bs->inventory[INVENTORY_NAILS] > 0) &&
		!(bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 0) &&
		!(bs->inventory[INVENTORY_EXPLOSIVESHOTGUN] > 0 && bs->inventory[INVENTORY_EXPLOSIVESHELLS] > 0) ) {
		return;
	}
	// try to avoid the beartrap
	//PKMOD - Ergodic - 08/30/01 - reduce avoid spot size from 160 to 120
	trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 120, AVOID_ALWAYS);
	//
	if (bs->numbeartraps >= MAX_BEARTRAPS)
		return;
	bs->beartraps[bs->numbeartraps] = state->number;
	bs->numbeartraps++;
}
...
//PKMOD - Ergodic 08/30/01 - new code for bots to avoid/target enemy sentrys
==================
*/
void BotCheckForAutosentrys(bot_state_t *bs, entityState_t *state) {
	// if this is not an autosentry
	if ( state->eType != ET_AUTOSENTRY_BASE || state->weapon != WP_SENTRY )
		return;
	// if this autosentry is from someone on our own team
	//PKMOD - Ergodic 10/10/03 - BOT AI will use modelindex instead of generic1 in order to free generic1 for "cell damages"
	//	if ( state->generic1 == BotTeam(bs) )
	if ( state->modelindex == BotTeam(bs) )
		return;
...
//	bs->inventory[INVENTORY_EXPLOSIVESHELLS] = bs->cur_ps.ammo[WP_EXPLODING_SHELLS];
	...
	if (!(bs->inventory[INVENTORY_PKA_NAILGUN] > 0 && bs->inventory[INVENTORY_NAILS] > 0) &&
		!(bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 0) &&
		!(bs->inventory[INVENTORY_EXPLOSIVESHOTGUN] > 0 && bs->inventory[INVENTORY_EXPLOSIVESHELLS] > 0) &&
		!(bs->inventory[INVENTORY_SHOTGUN] > 0 && bs->inventory[INVENTORY_SHELLS] > 0) && 
		!(bs->inventory[INVENTORY_RAILGUN] > 0 && bs->inventory[INVENTORY_SLUGS] > 0) ) {
		return;
	}
	// try to avoid the autosentry
	//PKMOD - Ergodic - 08/30/01 - set avoid spot size to 400
	trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 400, AVOID_ALWAYS);
	//
	if ( bs->numautosentrys >= MAX_AUTOSENTRYS )
		return;
	bs->autosentrys[bs->numautosentrys] = state->number;
	bs->numautosentrys++;
}
...
//PKMOD - Ergodic 08/30/01 - new code for bots to avoid Gravity Wells
==================
*/
void BotCheckForGravityWells(bot_state_t *bs, entityState_t *state) {
	// if this is not a Beartrap
	if ( state->eType != ET_GRAVITY_WELL || state->weapon != WP_SENTRY )
		return;

	// try to avoid the Gravity Well
	//PKMOD - Ergodic - 08/30/01 - set avoid spot size to 1000 [Note: gw radius is 1200 - grin]
	trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 1000, AVOID_ALWAYS);
	//
	if ( bs->numgravitywells >= MAX_GRAVITYWELLS )
		return;
	bs->gravitywells[bs->numgravitywells] = state->number;
	bs->numgravitywells++;
}
...
void BotCheckSnapshot(bot_state_t *bs) {
	...
	//PKMOD - Ergodic 08/30/01 - remove these fields in PKA
	/*++++
	//reset kamikaze body
	bs->kamikazebody = 0;
	...
	//PKMOD - Ergodic 04/14/01 - reset number of beartraps
	bs->numbeartraps = 0;
	//PKMOD - Ergodic 08/30/01 - reset number of autosentrys
	bs->numautosentrys = 0;
	//PKMOD - Ergodic 08/30/01 - reset number of gravitywells
	bs->numgravitywells = 0;
	...
	while( ( ent = BotAI_GetSnapshotEntity( bs->client, ent, &state ) ) != -1 ) {
		...
		//PKMOD - Ergodic 04/14/01 - check for beartraps which the bot should destroy
		BotCheckForAutosentrys(bs, &state);
		//PKMOD - Ergodic 04/14/01 - check for beartraps which the bot should destroy
		BotCheckForBeartraps(bs, &state);
		//
#ifdef MISSIONPACK
...
void BotSetupDeathmatchAI(void) {
	...
	//PKMOD - Ergodic 12/02/01 - debug: setting bot_dragon to 1 (inactive)
//	Com_Printf("BotSetupDeathmatchAI - setting bot_dragon to 1\n" );

//PKMOD - Ergodic 12/02/01 - set default to "1" so that bots will use grapple
//	trap_Cvar_Register(&bot_grapple, "bot_grapple", "1", 0);
//PKMOD - Ergodic 02/11/02 - change the cvar flags
//PKMOD - Ergodic 02/11/02 - move bot_grapple to g_bot.c:G_InitBots
//	trap_Cvar_Register(&bot_grapple, "bot_grapple", "1", CVAR_SERVERINFO | CVAR_USERINFO | CVAR_LATCH);
	...
	//PKMOD - Ergodic 12/02/01 - set default to "1" so that bots will use grapple
	trap_Cvar_Register(&g_spSkill, "bot_dragon", "1", CVAR_SERVERINFO | CVAR_USERINFO | CVAR_LATCH);
