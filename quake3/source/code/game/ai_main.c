...
//	Com_Printf("BotAIShutdownClient - attempting to shutdown bot client>%d<\n", client );

	bs = botstates[client];
	if (!bs || !bs->inuse) {
		//BotAI_Print(PRT_ERROR, "BotAIShutdownClient: client %d already shutdown\n", client);
		//PKMOD - Ergodic 02/13/04 - debug bot's restart (inactive)
		//Com_Printf("BotAIShutdownClient - client>%d< already shutdown\n", client );
		return qfalse;
	}

	if (restart) {
		//PKMOD - Ergodic 01/14/02 - debug bot's restart (inactive)
//		Com_Printf("BotAIShutdownClient - restarting client>%d<\n", client );
...
int BotInitLibrary(void) {
	...
	hold_maxclients = atoi( buf ) + MAX_PRIVATE_BOTS;
	trap_BotLibVarSet("maxclients", va( "%i", hold_maxclients ) );
	...
	//PKMOD - Ergodic 12/02/01 - initialize varibables for bots to use grapple
	trap_BotLibVarSet("weapindex_grapple", va("%d", WP_GRAPPLING_HOOK));
	trap_BotLibVarSet("entitytypemissile", va("%d", ET_MISSILE));
...
int BotAIShutdown( int restart ) {
	...
	//PKMOD - Ergodic 02/13/04 - debug bot's userinfo (inactive)
	//char netname[MAX_MESSAGE_SIZE];

	//PKMOD - Ergodic 01/15/02 - debug bot's restart (inactive)
//	Com_Printf("+-+-+-BotAIShutdown-+-+-+\n" );
	...
	if ( restart ) {
		...
		for (i = 0; i < MAX_CLIENTS; i++) {
			if (botstates[i] && botstates[i]->inuse) {
				...
				//PKMOD - Ergodic 02/13/04 - debug bot's userinfo (inactive)
				//ClientName(botstates[i]->client, netname, sizeof(netname));
				//Com_Printf("BotAIShutdown - clientnum>%d<, entitynum>%d<, name>%s<, flags>%d<\n", botstates[i]->client, botstates[i]->entitynum, netname, g_entities[botstates[i]->entitynum].r.svFlags );
