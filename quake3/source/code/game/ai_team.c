...
void BotTeamAI(bot_state_t *bs) {
	...
	//
	if ( gametype < GT_TEAM  ) {
		//PKMOD - Ergodic 03/30/02 - in a non-team game Private Bot will act as a team follower
		if ( g_entities[bs->entitynum].r.svFlags & SVF_PRIVATEBOT ) {
			//Does private bot have a teamleader set?
			if ( !BotValidTeamLeader(bs) ) {
				//Set owner as Private Bot leader
				ClientName(g_entities[bs->entitynum].parent->client->ps.clientNum, bs->teamleader, sizeof(bs->teamleader));
				return;
			}
		}
		return;
