...
void UpdateTournamentInfo( void ) {
  ...
  int score1, score2;
	...
	if ( level.clients[playerClientNum].sess.sessionTeam == TEAM_SPECTATOR ) {
...
#else
		//PKMOD - Ergodic 02/02/04 - calculate winner
		if (&level.clients[playerClientNum] == &level.clients[ level.sortedClients[0] ]) {
			winner = 1;
		} else {
			winner = 0;
		...
		//PKMOD - Ergodic 09/04/00 move painkiller to end of parameters
		//PKMOD - Ergodic 12/17/00 - place PERS_HUB_FLAG as first bit of PERS_PAINKILLER_COUNT
		//				remove the lowest bit of PERS_PAINKILLER_COUNT
		//PKMOD - Ergodic 02/02/04 - modify SP postgame argument list to sync with ui_atoms.c
		Com_sprintf( msg, sizeof(msg), "postgame %i %i %i %i %i %i %i %i %i %i %i", level.numNonSpectatorClients, playerClientNum, accuracy,
			...
			player->client->ps.persistant[PERS_GAUNTLET_FRAG_COUNT], perfect,
			(player->client->ps.persistant[PERS_PAINKILLER_COUNT] >> 1) / 10, player->client->ps.persistant[PERS_SCORE],
			level.time, winner );
#endif
	}
...
static gentity_t *SpawnModelOnVictoryPad( gentity_t *pad, vec3_t offset, gentity_t *ent, int place ) {
	...
	//PKMOD - Ergodic 09/20/00 - add proper stance for dragon carrier
	if ( ( body->s.weapon == WP_GAUNTLET)  || ( body->s.weapon == WP_GRAPPLING_HOOK ) ){
...
static void CelebrateStop( gentity_t *player ) {
	...
	//PKMOD - Ergodic 09/20/00 - add proper stance for dragon carrier
	if ( ( player->s.weapon == WP_GAUNTLET)  || ( player->s.weapon == WP_GRAPPLING_HOOK ) ){
		anim = TORSO_STAND2;
	}
	else {
		anim = TORSO_STAND;
