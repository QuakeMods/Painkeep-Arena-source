...
//PKMOD - 07/12/00 Ergodic set gameversion to "pkarena"
//PKMOD - Ergodic 02/01/01 - put version number on string
//			Note: adhere to a rule that "letter" changes to the version number
//				will be for server only changes, and will not need to be fully
//				compatible. But the server version must be higher than or equal
//				to the client version. Thus, client:"pkarena 2.1a" will be allowed
//				on servers "pkarena 2.1a", "pkarena 2.1b", etc.
//				Client:"pkarena 2.1b" will NOT be allowed on server "pkarena 2.1a".
//			The game modifier will need to determine when to change the
//			primary version number so that server and client are in sync. 
//
//		- 03/27/01 - this code does not function as planned. check is done soley
//				through server purity crc process. 
...
//PKMOD - Ergodic 12/19/02 - re-version from 2.9n to 3.0
...
#define	GAMEVERSION	"pkarena 3.0n"
...
struct gentity_s {
	...
	//PKMOD Ergodic - 03/28/00, add airfist fields to Client structure
	//PKMOD Ergodic - 07/30/00 AirFist_Level will also hold the item's 
	//					move state flag (1==moved , 0==not moved)

	int			AirFist_Level;			//{100==full, 0==empty}

	//PKMOD Ergodic - 10/03/00 AirFist_ResetTime will also hold the dragon's item state
	//					dragon state flag (1==holding an item , 0==not holding an item)
	int			AirFist_ResetTime;		//When next AirFist rebuild will occur
	//PKMOD Ergodic - 05/26/00, add beartrap immune time
	//PKMOD Ergodic - 04/07/01 - rename variable to add autosentry
	int			BearTrap_Autosentry_ImmuneTime;
	//PKMOD Ergodic - 07/30/00 add original location of item in map before it was moved
	//				by airfist, gravity well, or grapple
	vec3_t		pka_originalposition;
	//PKMOD Ergodic - 09/09/00, add Client strike opponent with lightning
	//PKMOD Ergodic - 12/28/00, change light_strike (flag) to general pka flag structure
	int			pka_flags;
	gentity_t	*Lightning_Strike_Enemy;
	//PKMOD Ergodic - 04/16/01, local gravity effects
	int			pka_gravity;
	int			pka_gravity_time;
...
typedef struct {
	...
	//			all Private Bots will be disconnected at the start of the next level.
	qboolean	sessionPrivateBot;
} clientSession_t;
...
//PKMOD - Ergodic 07/25/01 - structure to hold respawn points for
//			SP_info_player_deathmatch
#define				MAX_RESPAWN_POSITION_INDEX		75		//maximum respawn position
extern	int			Respawn_Position_Index;					//points to last Respawn_Position element 
extern	gentity_t	*Respawn_Positions[];
...
//PKMOD - Ergodic 03/07/02 - check for owner vs PB or PB vs owner team
qboolean PrivateBotOwnerTeam( gentity_t *ent1, gentity_t *ent2 );

//PKMOD Ergodic - 01/21/2001, create a debug version of radiusdamage for exploding shells
//qboolean PKA_RadiusDamage ( vec3_t origin, gentity_t *attacker, float damage, float radius, gentity_t *ignore, int mod, float u, float r);
...
//
//PKMOD Ergodic - 03/29/2000, AirFist constant definitions...
//
#define AIRFIST_DURATION 1000 //AirFist Blast duration is 1 second
#define AIRFIST_RESET_WAIT 1500 //must wait a 1.5 seconds to rebuild AirFist
...
#endif
//PKMOD - Ergodic 08/05/01 - add for call from g_mover
...
//PKMOD - Ergodic 04/19/01 - add definition for teleporter/sentry exploit fix
...
//PKMOD - Ergodic 06/11/02 - main think loop
...
gentity_t *fire_autosentry (gentity_t *self, vec3_t start, vec3_t aimdir);

//Ergodic - 04/16/03 - Add Shooter logic to function
gentity_t *fire_beartrap (gentity_t *self, vec3_t start, vec3_t aimdir, int shooterSpeed);

gentity_t *fire_chainlg (gentity_t *self, vec3_t start, vec3_t aimdir);
gentity_t *fire_airfist (gentity_t *self, vec3_t start, vec3_t dir, float mc_quadFactor);
gentity_t *fire_nailgun (gentity_t *self, vec3_t start, vec3_t aimdir);
...
//PKMOD - Ergodic 10/11/00 - hub vote teleport touch function
void trigger_hubvote_touch (gentity_t *self, gentity_t *other, trace_t *trace );

//PKMOD - Ergodic 08/26/03 - dynamic hub map vote teleport touch function
...
//PKMOD - Ergodic 11/19/01 - Remove this code
//void G_PrintWelcomeMessage( int clientNum );
...
//PKMOD - Ergodic 01/21/02 - modify cal to pass in Private Bot status
...
extern	vmCvar_t	g_hub_fraglimit;
extern	vmCvar_t	g_hub_timelimit;
//PKMOD - Ergodic 11/07/00 - add hub flag to make toggling easier
//PKMOD - Ergodic 12/25/00 - name change "g_hub_flag" to "hub_flag"
extern	vmCvar_t	hub_flag;
//PKMOD - Ergodic 01/20/01 - exploding shells muzzle displacement
//			debug usage only (fixed 01/22/01 - therefore unneeded)
//extern	vmCvar_t	x_explmuzzle;
//extern	vmCvar_t	y_explmuzzle;
//extern	vmCvar_t	z_explmuzzle;

//PKMOD - Ergodic 01/06/02 - Private Bot CVAR settings
extern	vmCvar_t	g_PrivateBotSkill;

//PKMOD - Ergodic 08/24/03 - Add Dynamic Hub cvars
extern	vmCvar_t	g_HubAltMap1;
extern	vmCvar_t	g_HubAltTitle1;
extern	vmCvar_t	g_HubAltMap2;
extern	vmCvar_t	g_HubAltTitle2;
extern	vmCvar_t	g_HubAltMap3;
extern	vmCvar_t	g_HubAltTitle3;
extern	vmCvar_t	g_HubAltMap4;
extern	vmCvar_t	g_HubAltTitle4;

//PKMOD - Ergodic 02/11/04 - add cvars for pka chat and pka tourney rules
extern	vmCvar_t	g_pkatourneychat;
extern	vmCvar_t	g_pkatourneyrules;
