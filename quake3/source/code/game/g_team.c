...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	//PKMOD - Ergodic 02/20/04 - debug generic1 (inactive)
	//Com_Printf( "Team_TouchOurFlag - generic1>%d<, enemy_flag>%d<\n", cl->ps.generic1, enemy_flag );

	//PKMOD - Ergodic 08/30/01 - if flag is mounted on dragon, then clear the drogondeploy
	//PKMOD - Ergodic 02/21/04 - fix infinite flag bug... mask off the first 15 weapon bits 
	//			especially mask off the "toggle bit" which is set in the bg_pmove.c
	if ( ( cl->ps.generic1 & 15 ) == enemy_flag )
		cl->ps.generic1 = 0;
	...
	// add the sprite over the player's head
	other->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST | EF_AWARD_DEFEND | EF_AWARD_CAP | EF_AWARD_PAINKILLER );
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (player->client->sess.sessionTeam !=
			...
			if (player->client->pers.teamState.lastreturnedflag + 
				...
				// add the sprite over the player's head
				player->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST | EF_AWARD_DEFEND | EF_AWARD_CAP | EF_AWARD_PAINKILLER );
...
============
*/
gentity_t *SelectCTFSpawnPoint ( team_t team, int teamstate, vec3_t origin, vec3_t angles, int client_hub_flag ) {
	gentity_t	*spot;
