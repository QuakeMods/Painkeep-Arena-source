...
//PKMOD - Ergodic 11/07/00 - enable setting hub limits in the user interface
extern vmCvar_t	ui_hub_fraglimit;
extern vmCvar_t	ui_hub_timelimit;
...
extern vmCvar_t	ui_scorePainkiller;
extern vmCvar_t	ui_scoreFrags;
...
extern vmCvar_t	ui_scoreMedalBonus;
...
//PKMOD - Ergodic 10/14/00 - 3/4's dimmer blue
extern vec4_t		color_blue_75;
...
//PKMOD - Ergodic 09/30/00 - voting menu
extern void UI_ConfirmVote( char *mapname, char *maptitle, void (*draw)( void ), void (*action)( qboolean result ) );
...
typedef struct {
	...
	//PKMOD - Ergodic 07/15/00 Connection Intro Morphing Sequence
	//PKMOD - Ergodic 03/28/01 - removed morphing sequence
	qhandle_t			menuPKAM014Shader;
...
//PKMOD - Ergodic 04/25/02 - set max uiArenaBOTS
#define MAX_uiArenaBOTS 8
...
#define AVERAGE_FRAGS_PER_MINUTE   2.8	 //PKMOD - Ergodic 02/03/04 _ used in calculating PAR score
...
typedef struct {
  ...
  const char *mapLoadName;
	...
	//PKMOD - Ergodic 04/24/02 - store arena's opponent bot names
	const char *BotList;
	//PKMOD - Ergodic 04/27/02 - store arena's fraglimt and timelimit
	const char *fragLimit;
...
typedef struct {
	...
	//PKMOD - Ergodic 11/08/02 - store bot information
	int	botCount;
	...
	//PKMOD - Ergodic 11/08/02 - store bot information
	char botNames[MAX_CLIENTS][MAX_NAME_LENGTH];
	...
	//PKMOD - Ergodic 12/06/02 - Add functionality to select and play music in the UI menu
	const char *musicList[MAX_MUSIC];
	int musicIndex;
	int musicCount;

	//PKMOD - Ergodic 09/11/03 - Add functionality for Server Hosted maps to be referenced
	//			in the HUB
	const char *hubAltList[MAX_HUBALT];
	int hubAltIndex;
	int hubAltCount;

	//PKMOD - Ergodic 09/12/03 - Add functionality for Server Hosted maps to be referenced
	//			in the HUB
//	const char *hubAltTitleList[MAX_HUBALT];
//	int hubAltTitleIndex;
//	int hubAltTitleCount;
...
//PKMOD - Ergodic 04/24/02 - store selected arena's bot names
typedef struct {
	char botname[MAX_HEADNAME];
} uiArenaBOTS_t;

//PKMOD - Ergodic 04/24/02 - store selected arena's bot names (MAX 8 bots shown per arena)
extern uiArenaBOTS_t uiArenaBOTS[MAX_uiArenaBOTS];
...
typedef enum {
	...
	AWARD_FRAGS,
	AWARD_PERFECT,
	AWARD_PAINKILLER //PKMOD - Ergodic 09/03/00 - Fix podium medal award
} awardType_t;
...
typedef struct postGameInfo_s {
	...
	int time;
	int timeBonus;
	//PKMOD - Ergodic 02/02/04 - rename ShutoutBonus to medalBonus 
	int medalBonus;
	int skillBonus;
	int baseScore;
