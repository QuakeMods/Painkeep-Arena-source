...
//PKMOD - Ergodic 04/26/02 - debug ui - use field to debug player icons only when arena changes
//int	latest_arena = 99;
//int	show_debugs = 0;
...
//PKMOD - Ergodic 04/24/02 - store selected arena's bot names (MAX 8 bots shown per arena)
uiArenaBOTS_t uiArenaBOTS[MAX_uiArenaBOTS];
...
static const char *skillLevels[] = {
	//PKMOD - Ergodic 09/14/2002 - modify skill setting names
//  "I Can Win",
//  "Bring It On",
//  "Hurt Me Plenty",
//  "Hardcore",
...
static const serverFilter_t serverFilters[] = {
	{"All", "" },
	//PKMOD - Ergodic 12/19/03 - Unify all PainKeep versions to "pkarena" game directory
	//{"PainKeep Arena 3", "pkarena3" },
	//{"PainKeep Arena (2.8)", "pkarena" },
	{"PainKeep Arena", "pkarena" },
	{"Quake 3 Arena", "" }
};
...
void AssetCache() {
	...
	//PKMOD - Ergodic 12/16/02 - Remove From PKA 3.0
...
//PKMOD Ergodic 12/22/03 - get map info index from name
static int UI_MapIndexFromName(const char *map) {
  int i;
...
PKMOD - Ergodic 01/01/04 - Add functionality to establish the SET Hub Alternates UI

===============
*/
static void UI_SetuphubAlt( int altindex ) {

	//********
	char buff1[512];

	//PKMOD - Ergodic 01/01/04 - used to determine Hub Alt cvar's value
	const char	*cvar;
	...
	//PKMOD - Ergodic 01/01/04 - build the g_HubAltMapX cvars' name
	cvar = va("g_HubAltMap%i", altindex );

	//PKMOD - Ergodic 01/01/04 - determine g_HubAltMapX cvars' values
	trap_Cvar_VariableStringBuffer(cvar, buff1, sizeof(buff1));

	map_index = altindex;

	//is the value not set?
	if (buff1[0] == '\0') {
		//PKMOD - Ergodic 01/02/04 - debug settings (inactive)
		//Com_Printf("In UI_SetuphubAlt - cvar g_HubAltMap%d is not set\n", altindex);

		//if not set then default to the ""altindex" map of the maplist
		if ( map_index >= uiInfo.mapCount ) {	//something is very wrong someplace
			map_index = 0;
		}
	} else {
		map_index = UI_MapIndexFromName( buff1 );
		if ( map_index > 1000 ) {	//can't find map name in list
			//PKMOD - Ergodic 01/02/04 - debug settings (inactive)
			//Com_Printf("In UI_SetuphubAlt - cvar g_HubAltMap%d >%s<is unknown\n", altindex, buff1);
			map_index = 0;
		}
	}
	...
	//map name
	trap_Cvar_Set( cvar, uiInfo.mapList[map_index].mapLoadName );

	//map title
	cvar = va("g_HubAltTitle%i", altindex );
	trap_Cvar_Set( cvar, uiInfo.mapList[map_index].mapName );

	//map index
	cvar = va("ui_hubalt%i", altindex );
	trap_Cvar_Set(cvar, va("%i", map_index));
...
//PKMOD - Ergodic 12/22/03 - Add functionality to set private bot skill in the UI
static void UI_DrawPBSkill(rectDef_t *rect, float scale, vec4_t color, int textStyle) {
  int i;
	i = trap_Cvar_VariableValue( "g_PrivateBotSkill" );
  if (i < 1 || i > numSkillLevels) {
    i = 1;
  }
  Text_Paint(rect->x, rect->y, scale, color, skillLevels[i-1],0, 0, textStyle);
...
static void UI_DrawTeamMember(rectDef_t *rect, float scale, vec4_t color, qboolean blue, int num, int textStyle) {
	...
	if (value <= 0) {
		...
		//PKMOD - Ergodic 12/08/02 - do not use Team Arena style Team info
//ergo		if (ui_actualNetGameType.integer >= GT_TEAM) {
//ergo			if (value >= uiInfo.characterCount) {
//ergo				value = 0;
//ergo			}
//ergo			text = uiInfo.characterList[value].name;
//ergo		} else {
...
//PKMOD - Ergodic 12/23/03 - Add functionality to set HUB Alternate maps in the UI
static void UI_DrawMapName(rectDef_t *rect, float scale, vec4_t color, qboolean blue, int num, int textStyle) {
	// 0 - None
	// 1 - Human
	// 2..NumCharacters - Bot
	int value = trap_Cvar_VariableValue(va("ui_hubalt%i", num));
	const char *text;
//	if (value <= 0) {
//		text = "Closed";
//	} else if (value == 1) {
...
//ergo			text = uiInfo.characterList[value].name;
//ergo		} else {
			if (value >= uiInfo.mapCount) {
				value = 0;
			}
			text = uiInfo.mapList[value].mapLoadName;
//ergo		}
//	}
  Text_Paint(rect->x, rect->y, scale, color, text, 0, 0, textStyle);
}
...
//PKMOD - Ergodic 11/28/02 - display of the map levelshot after game completes
static void UI_PKA_DrawMapPostview(rectDef_t *rect, float scale, vec4_t color, qboolean net) {
	char	buffer[MAX_STRING_CHARS];

	trap_Cvar_VariableStringBuffer("ui_imageMap", buffer, sizeof(buffer));
	if ( strlen( buffer ) ) {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, trap_R_RegisterShaderNoMip( buffer ) );
	} else {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, trap_R_RegisterShaderNoMip("menu/art/unknownmap"));
	}
}
 
//PKMOD - Ergodic 12/24/03 - display of the Hub Alt map levelshot
static void UI_PKA_DrawHubAltMap(rectDef_t *rect, float scale, vec4_t color, int mapindex) {

	//PKMOD - Ergodic 12/24/03 - debug value
	//Com_Printf( "mapindex is >%d<, levelshot>%d<\n", mapindex, uiInfo.mapList[mapindex].levelShot );

	if (uiInfo.mapList[mapindex].levelShot == -1) {
		uiInfo.mapList[mapindex].levelShot = trap_R_RegisterShaderNoMip(uiInfo.mapList[mapindex].imageName);
	}

	if ( mapindex < 0 ) {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, trap_R_RegisterShaderNoMip("menu/art/unknownmap"));
	}
	else if (uiInfo.mapList[mapindex].levelShot > 0) {
			UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, uiInfo.mapList[mapindex].levelShot);
		} else {
			UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, trap_R_RegisterShaderNoMip("menu/art/unknownmap"));
		}
...
//PKMOD - Ergodic 04/22/02 - update display of the selected map
static void UI_PKA_UpdatePreview(rectDef_t *rect, float scale, vec4_t color, qboolean net) {
	int arena = ui_currentMap.integer;

	char botopponents[MAX_INFO_KEY];
	char *bo, *ab;
	int botindex;
	int indx;
	int namelength;

	//PKMOD - Ergodic 04/26/02 - debug logic
//	if ( arena != latest_arena ) {
//		latest_arena = arena;
//		show_debugs = 1;
//	}

	//PKMOD - Ergodic 04/23/02 - debug update display of the selected map (inactive)
//	Com_Printf( "UI_PKA_UpdatePreview - selected map>%s<\n", uiInfo.mapList[ui_currentMap.integer].mapLoadName );		

	if (arena < 0 || arena > uiInfo.mapCount) {
		ui_currentMap.integer = 0;
		trap_Cvar_Set("ui_currentMap", "0");
		arena = 0;
	}

	if (uiInfo.mapList[arena].levelShot == -1) {
		uiInfo.mapList[arena].levelShot = trap_R_RegisterShaderNoMip(uiInfo.mapList[arena].imageName);
	}

	if (uiInfo.mapList[arena].levelShot > 0) {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, uiInfo.mapList[arena].levelShot);
	} else {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, trap_R_RegisterShaderNoMip("menu/art/unknownmap"));
	}

	//load Bot opponent names
	Q_strncpyz( botopponents, uiInfo.mapList[arena].BotList,  sizeof(botopponents) );

	//PKMOD - Ergodic 04/26/02 - debug bot opponents
//	if ( show_debugs )
//		Com_Printf( "UI_PKA_UpdatePreview - arena>%d<, opponents>%s<\n", arena, bo );  

	//initialize
	botindex = 0;
	namelength = 0;
	bo = botopponents;
	ab = &uiArenaBOTS[botindex].botname[0];

	//store selected arena's bot names into ArenaBOTS 

	for ( indx = 0; indx < strlen( botopponents ); indx++ ) {
		if ( *bo == ' ' ) {		//blank
			*ab = '\0';			//end of string delimter
			botindex++;
			if ( botindex >= MAX_uiArenaBOTS )
				break;
			namelength = 0;
			ab = uiArenaBOTS[botindex].botname;
//			if ( show_debugs )
//				Com_Printf( " - %d\n", botindex );  
		}
		else {
			namelength++;
			if ( namelength < ( MAX_HEADNAME - 1 ) )
				*ab++ = *bo;
//			if ( show_debugs )
//				Com_Printf( "%c", *bo );  
	...
	*ab = '\0';			//end of string delimter (last one)
//	if ( show_debugs ) {
//		Com_Printf( " - %d\n", botindex );
//		for ( indx = 0; indx <= botindex; indx++ ) {
//			Com_Printf( "uiArenaBOTS[%d] - >%s<\n", indx, &uiArenaBOTS[indx].botname[0] );
//		}
//	}



	//clear remaining uiArenaBOTS slots
	for ( indx = botindex + 1; indx < MAX_uiArenaBOTS; indx++ ) {
		uiArenaBOTS[indx].botname[0] = '\0';
	}

}						 

//PKMOD - Ergodic 04/24/02 - update display of the selected map's bot opponent heads
static void UI_PKA_DisplayBotHead(rectDef_t *rect, float scale, vec4_t color, qboolean net, int botnumber) {
	char		*botInfo;
	char		*model;
	qhandle_t	botIcon = 0; 

	if ( uiArenaBOTS[botnumber].botname[0] !=  '\0' ) {

		botInfo = UI_GetBotInfoByName( &uiArenaBOTS[botnumber].botname[0] );//	UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, uiInfo.mapList[map].levelShot);

		model = Info_ValueForKey( botInfo, "model" );
	
		//PKMOD - Ergodic 04/25/02 - debug stored Bot name and info
//		Com_Printf( "UI_PKA_DisplayBotHead - num>%d<, name>%s<, model>%s<, info>%s<\n", botnumber, &uiArenaBOTS[botnumber].botname[0], model, botInfo ); 

		botIcon = trap_R_RegisterShaderNoMip( va( "models/players/%s/icon_default.tga", model ) );

		//PKMOD - Ergodic 04/26/02 - handle variant bots
		if (!botIcon) {
			//determine if bot is a variant form a standard bot directory (ie , Phobos, Patriot, Wrack, etc)  
			char dirname[MAX_INFO_STRING];
			int indx = 0;
			char *s;

			//PKMOD - Ergodic 04/25/02 - debug stored Bot name and info (inactive)
//			Com_Printf( "UI_PKA_DisplayBotHead - num>%d<, name>%s<, model>%s<, info>%s<\n", botnumber, &uiArenaBOTS[botnumber].botname[0], model, botInfo ); 
			...
			while (1) {
				...
				if ( indx >= MAX_INFO_STRING ) {
					dirname[indx] = '\0';
					break;
				}
			}

			//PKMOD - Ergodic 04/25/02 - dirname (inactive)
//			Com_Printf( "UI_PKA_DisplayBotHead - dirname>%s<\n", dirname ); 

			botIcon = trap_R_RegisterShaderNoMip( va( "models/players/%s/icon_%s.tga", dirname, &uiArenaBOTS[botnumber].botname[0] ) );

			//if still not a valid icon then default to sarge (good old sarge!)
			if (!botIcon)
				botIcon = trap_R_RegisterShaderNoMip( "models/players/sarge/icon_default.tga" );
		}

		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, botIcon);
	}
...
//PKMOD - Ergodic 04/24/02 - update display of the selected map's Bot Text names
static void UI_PKA_DisplayBotText(rectDef_t *rect, float scale, vec4_t color, int textStyle, int botnumber) {

	Text_Paint(rect->x, rect->y, scale, color, va("%s", &uiArenaBOTS[botnumber].botname[0] ), 0, 0, textStyle);

}

//PKMOD - Ergodic 04/23/02 - display selected map name and title
static void UI_PKA_DisplayMapNameTitle(rectDef_t *rect, float scale, vec4_t color, int textStyle, int net ) {
//	int map = ui_currentMap.integer;
	...
	//PKMOD - Ergodic 05/02/02 - debug counter (inactive)
//	Com_Printf( "UI_PKA_DisplayMapNameTitle - currentNetMap>%d<, currentMap>%d<\n", ui_currentNetMap.integer, ui_currentMap.integer );  

	Text_Paint(rect->x, rect->y, scale, color, va("%s: %s", uiInfo.mapList[map].mapLoadName, uiInfo.mapList[map].mapName ), 0, 0, textStyle);
}
...
static void UI_DrawMapTimeToBeat(rectDef_t *rect, float scale, vec4_t color, int textStyle) {
	...
	//PKMOD - Ergodic 02/03/04 - debug (inactive)
	//Com_Printf( "UI_DrawMapTimeToBeat map>%d< timetobeat>%d<\n", ui_currentMap.integer, uiInfo.mapList[ui_currentMap.integer].timeToBeat[0] );
	...
	//PKMOD - Ergodic 02/04/04 - set time to beat directly from the fraglimit
	//time = uiInfo.mapList[ui_currentMap.integer].timeToBeat[0];
	

	//PKMOD - Ergodic 02/03/04 - if store the PAR time based upon fraglimit and frags_per_minute
	time = 60 * atoi( uiInfo.mapList[ui_currentMap.integer].fragLimit ) / AVERAGE_FRAGS_PER_MINUTE;	//time in seconds
	minutes = time / 60;
	seconds = time % 60;
...
//PKMOD - Ergodic 04/27/02 - update display of the selected map's fraglimit
//PKMOD - Ergodic 09/15/02 - move SP bot infomation to this module [NOTE: SP ONLY!]
static void UI_PKA_DisplayFragLimit( rectDef_t *rect, float scale, vec4_t color, int textStyle ) {
	int arena = ui_currentMap.integer;
...
//	Com_Printf( "UI_PKA_UpdatePreview - selected map>%s<\n", uiInfo.mapList[ui_currentMap.integer].mapLoadName );		
	...
	//PKMOD - Ergodic 09/14/02 - debug (inactive)
//	Com_Printf("UI_PKA_DisplayFragLimit - arean>%d<, fraglimit>%d<\n", arena, uiInfo.mapList[arena].fragLimit ); 

	Text_Paint(rect->x, rect->y, scale, color, va("%s", uiInfo.mapList[arena].fragLimit ), 0, 0, textStyle);
...
//PKMOD - Ergodic 04/27/02 - update display of the selected map's timelimit
static void UI_PKA_DisplayTimeLimit( rectDef_t *rect, float scale, vec4_t color, int textStyle ) {
	int arena = ui_currentMap.integer;

	Text_Paint(rect->x, rect->y, scale, color, va("%s", uiInfo.mapList[arena].timeLimit ), 0, 0, textStyle);
}
...
static void UI_DrawPlayerModel(rectDef_t *rect) {
  ...
  char team[256];
	...
	if (trap_Cvar_VariableValue("ui_Q3Model")) {
		...
		//PKMOD - Ergodic 02/06/04 - setting team_model create an error message in the 
		//		console - so don't set team_model
		//strcpy(team, UI_Cvar_VariableString("ui_teamName"));
		team[0] = '\0'; //PKMOD - 02/06/04 - set_team model to null
...
static int UI_OwnerDrawWidth(int ownerDraw, float scale) {
  ...
  switch (ownerDraw) {
    ...
    case UI_HUBALT1:
		...
		case UI_HUBALT3:
		case UI_HUBALT4:
			value = trap_Cvar_VariableValue(va("ui_hubalt%i", ownerDraw-UI_HUBALT1 + 1));
//			if (value <= 0) {
//				text = "Closed";
//			} else if (value == 1) {
//				text = "Human";
...
//				value -= 2;
				...
				text = uiInfo.mapList[value].mapLoadName;
//			}
			s = va("%i. %s", ownerDraw-UI_HUBALT1 + 1, text);
			//debug inactive - i don't think this code is ever executed (ergodic)
			//Com_Printf("In ownerdraw\n");
      break;
...
//	int game = trap_Cvar_VariableValue("g_gametype");
	const char *text = "";
	//PKMOD - Ergodic 11/12/02 - remove this sction of code - it works improperly in the PKA UI
//	if (game >= GT_TEAM) {
...
/*
===============
PKMOD - Ergodic 11/08/02 - populate uiInfo with bot information
UI_BuildBotList
===============
*/
static void UI_BuildBotList() {
	int		n, count;
	int		isBot;
	char	info[MAX_INFO_STRING];
	...
	for( n = 0; n < count; n++ ) {
		...
		if ( info[0] && isBot ) {
			Q_strncpyz( uiInfo.botNames[uiInfo.botCount], Info_ValueForKey( info, "n" ), MAX_NAME_LENGTH );
			Q_CleanStr( uiInfo.botNames[uiInfo.botCount] );
			uiInfo.botCount++;
		}
	}
	
	//PKMOD - Ergodic 11/09/02 - debug selected bot (inactive)
//	n = trap_Cvar_VariableValue("cg_selectedPlayer");
//	Com_Printf( "UI_BuildBotList - botCount>%d<, cg_selectedPlayer>%d<\n", uiInfo.botCount, n );

/*
	n = trap_Cvar_VariableValue("cg_selectedPlayer");
	if (n < 0 || n > uiInfo.botCount) {
		n = 0;
	}
	if (n < count) {
		trap_Cvar_Set("cg_selectedPlayerName", uiInfo.botNames[n]);
	}
*/
}
...
//PKMOD - Ergodic 11/08/02 - Draw a Bot name that is selected for removal
static void UI_DrawBotName2Remove(rectDef_t *rect, float scale, vec4_t color, int textStyle) {
	int selected;

	if (uiInfo.uiDC.realTime > uiInfo.playerRefresh) {
		uiInfo.playerRefresh = uiInfo.uiDC.realTime + 8000;
		UI_BuildBotList();
	}
	if ( uiInfo.botCount == 0 )
		trap_Cvar_Set("cg_selectedPlayerName", "" );
	else {
		selected = trap_Cvar_VariableValue("cg_selectedPlayer");
		if (selected >= uiInfo.botCount) {
			selected = uiInfo.botCount - 1;
		} else if (selected < 0) {
			selected = 0;
		}
	 	trap_Cvar_Set( "cg_selectedPlayerName", uiInfo.botNames[selected]);
	}
...
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vec4_t color, qhandle_t shader, int textStyle) {
  ...
  switch (ownerDraw) {
    ...
    case UI_MAPPREVIEW:
      ...
      break;
  	//PKMOD - Ergodic 11/27/02 - display of the map levelshot after game completes
	case UI_MAPPOSTVIEW:
		UI_PKA_DrawMapPostview(&rect, scale, color, qtrue);
		break;
	//PKMOD - Ergodic 04/22/02 - update the display of the map preview
	case UI_UPDATEPREVIEW:
		UI_PKA_UpdatePreview(&rect, scale, color, qtrue);
		break;
  	//PKMOD - Ergodic 04/22/02 - display the selected map name and map title
	case UI_MAPNAMETITLE:
		UI_PKA_DisplayMapNameTitle(&rect, scale, color, qtrue, qfalse);
		break;
  	//PKMOD - Ergodic 04/22/02 - display the selected map name and map title
	case UI_MAPNAMETITLENET:
		UI_PKA_DisplayMapNameTitle(&rect, scale, color, qtrue, qtrue);
		break;
	//PKMOD - Ergodic 04/24/02 - display the (1) Single player Bot opponent head
	case UI_BOTHEAD0:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 0 );
		break;
	case UI_BOTHEAD1:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 1 );
		break;
	case UI_BOTHEAD2:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 2 );
		break;
	case UI_BOTHEAD3:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 3 );
		break;
	case UI_BOTHEAD4:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 4 );
		break;
	case UI_BOTHEAD5:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 5 );
		break;
	case UI_BOTHEAD6:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 6 );
		break;
	case UI_BOTHEAD7:
		UI_PKA_DisplayBotHead( &rect, scale, color, qtrue, 7 );
		break;
	//PKMOD - Ergodic 04/26/02 - display the (1) Single player Bot opponent's name
	case UI_BOTTEXT0:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 0 );
		break;
	case UI_BOTTEXT1:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 1 );
		break;
	case UI_BOTTEXT2:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 2 );
		break;
	case UI_BOTTEXT3:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 3 );
		break;
	case UI_BOTTEXT4:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 4 );
		break;
	case UI_BOTTEXT5:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 5 );
		break;
	case UI_BOTTEXT6:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 6 );
		break;
	case UI_BOTTEXT7:
		UI_PKA_DisplayBotText( &rect, scale, color, qtrue, 7 );
		break;
	//PKMOD - Ergodic 04/27/02 - update display of the selected map's fraglimit
	case UI_MAPFRAGLIMIT:
		UI_PKA_DisplayFragLimit( &rect, scale, color, qtrue );
		break;
	//PKMOD - Ergodic 04/27/02 - update display of the selected map's timelimit
	case UI_MAPTIMELIMIT:
		UI_PKA_DisplayTimeLimit( &rect, scale, color, qtrue );
		break;
//PKMOD - Ergodic 12/24/03 - display of the Hub Alt map levelshot
	case UI_HUBALTPREVIEW1:
		case UI_HUBALTPREVIEW2:
		case UI_HUBALTPREVIEW3:
		case UI_HUBALTPREVIEW4:
			value = trap_Cvar_VariableValue(va("ui_hubalt%i", ownerDraw-UI_HUBALTPREVIEW1 + 1));
			UI_PKA_DrawHubAltMap( &rect, scale, color, value ); 
		break;
	...
	//PKMOD - Ergodic 12/22/03 - Add functionality to set private bot skill in the UI
    case UI_PRIVATEBOTSKILL:
      UI_DrawPBSkill(&rect, scale, color, textStyle);
      break;
	//PKMOD - Ergodic 12/23/03 - Add functionality to set HUB Alternate maps in the UI
    case UI_HUBALT1:
		case UI_HUBALT2:
		case UI_HUBALT3:
		case UI_HUBALT4:
			//PKMOD - Ergodic 01/01/04 - determine if map name is nulllAdd functionality to set HUB Alternate maps in the UI
			UI_SetuphubAlt( ownerDraw - UI_HUBALT1 + 1 );
			UI_DrawMapName(&rect, scale, color, qtrue, ownerDraw - UI_HUBALT1 + 1, textStyle);
	break;
	case UI_BLUETEAMNAME:
    ...
    case UI_NETMAPCINEMATIC:
      ...
      UI_DrawNetFilter(&rect, scale, color, textStyle);
		...
		case UI_MAPS_SELECTION:
			...
			//PKMOD - Ergodic 02/03/04 - Load best game info
			if (ui_currentMap.integer >= 0 && ui_currentMap.integer < uiInfo.mapCount) {
				UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum);
				//set best time
				//PKMOD - Ergodic 02/03/04 - Remove gametype from saved file name
				//uiInfo.mapList[ui_currentMap.integer].timeToBeat[uiInfo.gameTypes[ui_gameType.integer].gtEnum] = ui_scoreTimeRaw.integer;
				//uiInfo.mapList[ui_currentMap.integer].timeToBeat[0] = ui_scoreTimeRaw.integer;
			}
			break;
		case UI_OPPONENT_NAME:
		...
		case UI_BOTNAME:
			...
			break;
		//PKMOD - Ergodic 11/08/02 - Draw botname that is selected for removal
		case UI_BOTNAME2REMOVE:
			UI_DrawBotName2Remove(&rect, scale, color, textStyle);
			break;
		case UI_BOTSKILL:
...
static qboolean UI_GameType_HandleKey(int flags, float *special, int key, qboolean resetMap) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER) {
		...
		//int holder; //debug (inactive)

		//PKMOD - Ergodic 02/05/04 - include gametype == 2 (singleplayer game) in the list 
		//		Debug	(inactive)
		/*
		if ( key == K_MOUSE2 ) 
			holder = 1;
		...
		Com_Printf( "UI_GameType_HandleKey K_MOUSE2>%d<, in ui_gametype>%d<", holder, ui_gameType.integer );
		*/ //ergodic 02/06/04

		//PKMOD - Ergodic 02/06/04 - change code so that gametype == 2 will be available
/*
...
//PKMOD - Ergodic 12/22/03 - Add functionality to set private bot skill in the UI
static qboolean UI_PBSkill_HandleKey(int flags, float *special, int key) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER) {
  	int i = trap_Cvar_VariableValue( "g_PrivateBotSkill" );
    ...
    trap_Cvar_Set("g_PrivateBotSkill", va("%i", i));
    return qtrue;
  }
  return qfalse;
}
...
//PKMOD - Ergodic 12/23/03 - Add functionality to set HUB Alternate maps in the UI
static qboolean UI_HubAlt_HandleKey(int flags, float *special, int key, qboolean blue, int num) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER) {
		char *cvar = va("ui_hubalt%i", num);
		...
		if (value >= uiInfo.mapCount) {
			...
			value = uiInfo.mapCount - 1;
		}

		trap_Cvar_Set(cvar, va("%i", value));

		//PKMOD - Ergodic 01/02/04 - debug settings (inactive)
		//Com_Printf("In qboolean UI_HubAlt_HandleKey num>%d<, value>%d<\n", num, value);

		//PKMOD - Ergodic 12/31/03 - set the title cvar
		cvar = va("g_HubAltTitle%i", num );
		trap_Cvar_Set( cvar, uiInfo.mapList[value].mapName );

		//PKMOD - Ergodic 01/01/04 - set the mapname cvar
		cvar = va("g_HubAltMap%i", num );
		trap_Cvar_Set( cvar, uiInfo.mapList[value].mapLoadName );
...
static qboolean UI_TeamMember_HandleKey(int flags, float *special, int key, qboolean blue, int num) {
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER) {
		...
		//PKMOD - Ergodic 12/08/02 - do not use Team Arena style Team info
//ergo		if (ui_actualNetGameType.integer >= GT_TEAM) {
//ergo			if (value >= uiInfo.characterCount + 2) {
//ergo				value = 0;
//ergo			} else if (value < 0) {
//ergo				value = uiInfo.characterCount + 2 - 1;
//ergo			}
//ergo		} else {
...
//PKMOD - Ergodic 11/08/02 - Draw selected botname
static qboolean UI_SelectedBotname_HandleKey(int flags, float *special, int key) {
  ...
  if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER) {
		...
		if (selected >= uiInfo.botCount) {
			...
			selected = uiInfo.botCount;
		}

		if ( uiInfo.botCount > 0 ) {		//1 or more bots
		 	trap_Cvar_Set( "cg_selectedPlayerName", uiInfo.botNames[selected]);
		 	trap_Cvar_Set( "cg_selectedPlayer", va("%d", selected));
		}
		else {
		 	trap_Cvar_Set( "cg_selectedPlayerName", "");
		 	trap_Cvar_Set( "cg_selectedPlayer", va("%d", selected));	//should be 0
		}
...
static qboolean UI_OwnerDrawHandleKey(int ownerDraw, int flags, float *special, int key) {
  switch (ownerDraw) {
	...
	//PKMOD - Ergodic 12/22/03 - Add functionality to set private bot skill in the UI
    case UI_PRIVATEBOTSKILL:
      return UI_PBSkill_HandleKey(flags, special, key);
      break;
	//PKMOD - Ergodic 12/23/03 - Add functionality to set HUB Alternate maps in the UI
    case UI_HUBALT1:
		case UI_HUBALT2:
		case UI_HUBALT3:
		case UI_HUBALT4:
	      UI_HubAlt_HandleKey(flags, special, key, qtrue, ownerDraw - UI_HUBALT1 + 1);
      break;
    case UI_BLUETEAMNAME:
    ...
    case UI_REDTEAM1:
      ...
      UI_NetFilter_HandleKey(flags, special, key);
		...
		//PKMOD - Ergodic 11/08/02 - Draw botname that is selected for removal
		case UI_BOTNAME2REMOVE:
			UI_SelectedBotname_HandleKey(flags, special, key);
			break;
		case UI_BOTSKILL:
...
PKMOD - Ergodic 12/06/02 - Add functionality to select and play music in the UI menu
PKMOD - Ergodic 12/07/02 - Add "music/" path information
===============
*/
static void UI_LoadMusic() {
	char	musiclist[4096];
	char	*musicname;
	int		i, len;

	uiInfo.musicCount = trap_FS_GetFileList( "music", ".wav", musiclist, 4096 );

	if (uiInfo.musicCount) {
		if (uiInfo.musicCount > MAX_MUSIC) {
			uiInfo.musicCount = MAX_MUSIC;
		}
		musicname = musiclist;
		for ( i = 0; i < uiInfo.musicCount; i++ ) {
			len = strlen( musicname );
			if (!Q_stricmp(musicname +  len - 4,".wav")) {
				musicname[len-4] = '\0';
			}
			Q_strupr( musicname );
			uiInfo.musicList[i] = String_Alloc( musicname );
			musicname += len + 1;
		}
	}
...
PKMOD - Ergodic 09/12/03 - Add functionality for Server Hosted maps to be referenced in the HUB

===============
*/
static void UI_LoadhubAlts() {

	//********
	char buff1[MAX_HUB_NAME];
	memset(buff1, 0, sizeof(buff1));

	uiInfo.hubAltCount = 0;

	// g_HubAltMap1 & g_hubAltTitle1
	memset(buff1, 0, sizeof(buff1));
	trap_Cvar_VariableStringBuffer("ui_HubAltDisp1", buff1, sizeof(buff1));
	if (buff1[0] != '\0') {

		uiInfo.hubAltList[uiInfo.hubAltCount] = String_Alloc( buff1 );
	...
	memset(buff1, 0, sizeof(buff1));
	trap_Cvar_VariableStringBuffer("ui_HubAltDisp2", buff1, sizeof(buff1));
	if (buff1[0] != '\0') {
	...
	// g_HubAltMap3 & g_hubAltTitle3
	memset(buff1, 0, sizeof(buff1));
	trap_Cvar_VariableStringBuffer("ui_HubAltDisp3", buff1, sizeof(buff1));
	if (buff1[0] != '\0') {
	...
	memset(buff1, 0, sizeof(buff1));
	trap_Cvar_VariableStringBuffer("ui_HubAltDisp4", buff1, sizeof(buff1));
	if (buff1[0] != '\0') {
...
===============
*/
static void UI_VotehubAlts() {
	uiClientState_t	cs;

	//debug selection (inactive)
//	Com_Printf( "Inside UI_VotehubAlts, selection is %d\n", uiInfo.hubAltIndex );

	trap_GetClientState( &cs );

	trap_Cmd_ExecuteText( EXEC_APPEND, va("hubaltvote %i %i\n", cs.clientNum, uiInfo.hubAltIndex ) );
...
PKMOD - Ergodic 09/12/03 - Add functionality for Server Hosted maps to be referenced in the HUB
		Here: the user does not vote, but opts to cancel vote

===============
*/
static void UI_NoVotehubAlts() {
	uiClientState_t	cs;

	//debug selection (inactive)
//	Com_Printf( "Inside UI_NoVotehubAlts, selection is -1 by default\n" );

	trap_GetClientState( &cs );

	trap_Cmd_ExecuteText( EXEC_APPEND, va("hubaltvote %i -1\n", cs.clientNum ) );
...
//PKMOD - Ergodic 09/17/02 - store variables for Single Player statistics 
static void UI_StartGameSP() {
	int g, temp;
	float skill;
	char buff[MAX_STRING_CHARS];

//PKMOD - Ergodic 02/04/04 - debug pre-SP game state (inactive)
	//int	spindex;
	...
	//PKMOD - Ergodic 02/05/04 - enforce gametype is set to single player
	...
	//PKMOD - Ergodic 11/27/02 - store map levelshot
	trap_Cvar_Set("ui_imageMap", uiInfo.mapList[ui_currentMap.integer].imageName );

	trap_Cvar_Set("ui_singlePlayerActive", "1");

	//PKMOD - Ergodic 12/17/02 - store map index
	trap_Cvar_Set("ui_spIndex", va("%i", ui_currentMap.integer ) );

	//PKMOD - Ergodic 09/18/02 - new code to correctly store SP game fraglimit
	// set up sp overrides, will be replaced on postgame
	//	temp = trap_Cvar_VariableValue( "fraglimit" );
	temp = atoi( uiInfo.mapList[ui_currentMap.integer].fragLimit );
	trap_Cvar_Set("ui_saveFragLimit", va("%i", temp));

//PKMOD - Ergodic 02/04/04 - debug pre-SP game state (inactive)
//	spindex = trap_Cvar_VariableValue("ui_spIndex");
	//PKMOD - Ergodic 02/04/04 - spindex
//	Com_Printf("UI_StartGameSP - spindex>%d<\n", spindex );
...
//PKMOD - Ergodic 12/17/02 - Replay Single Player game 
static void UI_ReplayGameSP() {
	int g, temp;
	float skill;
	char buff[MAX_STRING_CHARS];
	int	spindex;
	...
	//PKMOD - Ergodic 12/17/02 - debug (inactive)
	//Com_Printf( "UI_ReplayGameSP - spindex>%d<\n", spindex );
	//Com_Printf( "UI_ReplayGameSP - spindex map>%s<\n", uiInfo.mapList[spindex].mapLoadName );
	...
	trap_Cmd_ExecuteText( EXEC_APPEND, va( "spmap %s\n", uiInfo.mapList[spindex].mapLoadName ) );
	skill = trap_Cvar_VariableValue( "g_spSkill" );
	trap_Cvar_Set("ui_scoreMap", uiInfo.mapList[spindex].mapName);
	//PKMOD - Ergodic 11/27/02 - store map levelshot
	trap_Cvar_Set("ui_imageMap", uiInfo.mapList[spindex].imageName );
	...
	// set up sp overrides, will be replaced on postgame
	//	temp = trap_Cvar_VariableValue( "fraglimit" );
	temp = atoi( uiInfo.mapList[spindex].fragLimit );
	trap_Cvar_Set("ui_saveFragLimit", va("%i", temp));
...
*/
	...
	if (trap_Cvar_VariableValue("ui_recordSPDemo")) {
		Com_sprintf(buff, MAX_STRING_CHARS, "%s_%i", uiInfo.mapList[spindex].mapLoadName, g);
		trap_Cvar_Set("ui_recordSPDemoName", buff);
	}
...
#endif
			UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum);
		//PKMOD - Ergodic 04/22/02 - add new SP start command
		//PKMOD - Ergodic 09/17/02 - call a function so that cvar variables will be set 
		} else if (Q_stricmp(name, "loadGameSP") == 0) {
			UI_StartGameSP();
		//PKMOD - Ergodic 09/17/02 - call a replay SP game function 
		} else if (Q_stricmp(name, "replayGameSP") == 0) {
			UI_ReplayGameSP();
		//PKMOD - Ergodic 09/17/02 - call a play the NEXT SP game function 
		} else if (Q_stricmp(name, "nextGameSP") == 0) {
			int	spindex;
			...
			UI_LoadMods();
		//PKMOD - Ergodic 12/07/02 - Add functionality to select and play music in the UI menu
		} else if (Q_stricmp(name, "LoadMusic") == 0) {
			UI_LoadMusic();

		//PKMOD - Ergodic 09/12/03 - Add functionality for Server Hosted maps to be referenced
		//			in the HUB
		} else if (Q_stricmp(name, "LoadHubAlts") == 0) {
			UI_LoadhubAlts();

		//PKMOD - Ergodic 09/12/03 - Add functionality for Server Hosted maps to be referenced
		//			in the HUB
		} else if (Q_stricmp(name, "VoteHubAlts") == 0) {
			UI_VotehubAlts();

		//PKMOD - Ergodic 09/16/03 - Add functionality for Server Hosted maps to be referenced
		//			in the HUB
		//			Here: the user does not vote, but opts to cancel vote
		} else if (Q_stricmp(name, "NoVoteHubAlts") == 0) {
			UI_NoVotehubAlts();
...
//			}
		//PKMOD - Ergodic 11/11/02 - add functionality to UI for bot removal 
		} else if (Q_stricmp(name, "removeBot") == 0) {
			//PKMOD - Ergodic 11/11/02 - debug removeBot (inactive)
//			Com_Printf( "UI_RunMenuScript - trying to remove >%s<\n", UI_Cvar_VariableString("cg_selectedPlayerName") );

			if ( Q_stricmp( UI_Cvar_VariableString("cg_selectedPlayerName"), "" ) != 0 ) {
				//PKMOD - Ergodic 11/11/02 - debug removeBot (inactive)
//				Com_Printf( "UI_RunMenuScript - kicking >%s<\n", UI_Cvar_VariableString("cg_selectedPlayerName") );
			...
			*/
			...
			if (String_Parse(args, &orders)) {
				...
				if (selectedPlayer < uiInfo.myTeamCount) {
					//PKMOD - Ergodic 11/12/02 - Debug orders (inactive)
					//Com_Printf( "Q_stricmp(name, orders) - LT, selectedPlayer>%d<, uiInfo.teamNames[]>%s<\n", selectedPlayer, uiInfo.teamNames[selectedPlayer] );
					strcpy(buff, orders);
					...
					for (i = 0; i < uiInfo.myTeamCount; i++) {
						...
						//PKMOD - Ergodic 11/12/02 - Debug orders (inactive)
						//Com_Printf( "Q_stricmp(name, orders) - GT, i>%d<, selectedPlayer>%d<, uiInfo.teamNames[]>%s<\n", i, selectedPlayer, uiInfo.teamNames[i] );

						strcpy(buff, orders);
						//PKMOD - Ergodic 11/14/02 - Fix team player client number
		...
		//PKMOD - Ergodic 12/16/02 - Add non-judgemental Private Bot order command 
		} else if (Q_stricmp(name, "privatebotOrders") == 0) {
			const char *orders;
			if (String_Parse(args, &orders)) {
...
//	qboolean startRefresh = qtrue; TTimo: unused
	...
	if (len == 0) {
		strcpy(uiInfo.serverStatus.motd, "Welcome to PainKeep Arena!");
		len = strlen(uiInfo.serverStatus.motd);
	} 
...
static int UI_FeederCount(float feederID) {
	...
	//PKMOD - Ergodic 11/30/02 - Add functionality to assign and display headmodels 
	} else if (feederID == FEEDER_Q3HEADMODELS) {
		return uiInfo.q3HeadCount;
	} else if (feederID == FEEDER_CINEMATICS) {
		return uiInfo.movieCount;
	//PKMOD - Ergodic 12/06/02 - Add functionality to select and play music in the UI menu
	} else if (feederID == FEEDER_MUSIC) {
		return uiInfo.musicCount;

	//PKMOD - Ergodic 09/11/03 - Add functionality for Server Hosted maps to be referenced
	//			in the HUB
	} else if (feederID == FEEDER_HUBALTS) {
		return uiInfo.hubAltCount;
...
static const char *UI_FeederItemText(float feederID, int index, int column, qhandle_t *handle) {
	...
	*handle = -1;
	...
	//PKMOD - Ergodic 12/06/02 - Add functionality to select and play music in the UI menu
		...
		if (index >= 0 && index < uiInfo.musicCount) {
			return uiInfo.musicList[index];
		}

	//PKMOD - Ergodic 09/11/03 - Add functionality for Server Hosted maps to be referenced
	//			in the HUB
	} else if (feederID == FEEDER_HUBALTS) {
		if (index >= 0 && index < uiInfo.hubAltCount) {
			return uiInfo.hubAltList[index];
		}
...
static void UI_FeederSelection(float feederID, int index) {
  ...
  if (feederID == FEEDER_HEADS) {
	...
	//PKMOD - Ergodic 12/06/02 - Add functionality to select and play music in the UI menu
		...
		uiInfo.musicIndex = index;
		//PKMOD - Ergodic 12/07/02 - Build the dir/file string...
		strcpy( path, "MUSIC/" );
		strcat( path, uiInfo.musicList[index] );

		//PKMOD - Ergodic 12/07/02 - Play the music...
		trap_S_StopBackgroundTrack();
		trap_S_StartBackgroundTrack( path, path );
//		if (uiInfo.previewMusic >= 0) {
//		  trap_CIN_StopCinematic(uiInfo.previewMusic);
//		}
//		uiInfo.previewMusic = -1;

	//PKMOD - Ergodic 09/11/03 - Add functionality for Server Hosted maps to be referenced
	//			in the HUB
	} else if (feederID == FEEDER_HUBALTS) {

		uiInfo.hubAltIndex = index;
		//PKMOD - Ergodic 09/12/03 - debug selection (inactive)
		//Com_Printf( "Inside Feeder_hubalts, selection is %d\n", index );
...
static qboolean GameType_Parse(char **p, qboolean join) {
	...
	//PKMOD - Ergodic 05/06/02 - debug missing CTF gametype (inactive)
//	Com_Printf( "GameType_Parse - *p>%s<\n" , *p );
...
static qboolean MapList_Parse(char **p) {
	...
	while ( 1 ) {
		...
		if (token[0] == '{') {
			...
			//PKMOD - Ergodic 04/19/02 - do not use "_small" version of leveleshot 
...
void _UI_Init( qboolean inGameLoad ) {
	...
	//PKMOD - Ergodic 04/30/02 - force currentmap to zero for Single Player UI map preview
	ui_currentMap.integer = 0;
	trap_Cvar_Set("ui_currentMap", "0");

	//PKMOD - Ergodic 05/04/02 - force currentNetmap to zero for Designer UI map preview
	ui_currentNetMap.integer = 0;
	trap_Cvar_Set("ui_currentNetMap", "0");

	//PKMOD - Ergodic 02/04/04 - force version so that it willl properly be displayed on main Menu
	trap_Cvar_Set("ui_pkaVersion", PKARENA_VERSION);
  ...
  uiInfo.aliasCount = 0;
	...
	UI_ParseGameInfo("demogameinfo.txt");
#else
	//PKMOD - Ergodic 12/08/02 - do not use Team Arena style Team info file
    ...
    menuDef_t *menu = Menu_GetFocused();
		if (menu) {
			if (key == K_ESCAPE && down && !Menus_AnyFullScreenVisible()) {
				//PKMOD - Ergodic 09/17/03 - debug escape key in hubalternates menu (inactive)
				//Com_Printf( "_UI_KeyEvent - in:if (menu)/key == K_ESCAPE | menu>%s<\n", menu->window.name );
				//PKMOD - Ergodic 09/18/03 - call not vote function after escaping from hubalternates menu
				if ( !strcmp( menu->window.name, "hubalternates" ) ) {
					Menus_CloseAll();
					UI_NoVotehubAlts();
				}
				else {
...
void _UI_SetActiveMenu( uiMenuCommand_t menu ) {
  ...
  if (Menu_Count() > 0) {
	...
	//PKMOD - Ergodic 09/12/03 - Add functionality for Server Hosted maps to be referenced
	//			in the HUB
	  case UIMENU_HUBALTERNATES:
			trap_Cvar_Set( "cl_paused", "1" );
			trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			Menus_ActivateByName("hubalternates");
		  return;
	  }
  }
}
...
void UI_DrawConnectScreen( qboolean overlay ) {
	...
	//PKMOD - Ergodic 12/19/02 - was ystart + 48 to fix vertical text overlays in Online Game 
	//PKMOD - Ergodic 12/16/02 - Add beta logo
	//PKMOD - Ergodic 12/15/03 - Remove beta logo
	Text_PaintCenter(centerPoint, yStart + 24, scale, colorWhite, va("PainKeep Arena"), ITEM_TEXTSTYLE_SHADOWEDMORE);

	if (!Q_stricmp(cstate.servername,"localhost")) {
		//PKMOD - Ergodic 12/16/02 - was yStart + 48
		//PKMOD - Ergodic 12/19/02 - Return to 48 (was 72) to fix vertical text overlays in Online Game 
		Text_PaintCenter(centerPoint, yStart + 48, scale, colorWhite, va("Feel the Pain..."), ITEM_TEXTSTYLE_SHADOWEDMORE);
	} else {
...
//PKMOD - Ergodic 11/07/00 - enable setting of hub limits
vmCvar_t	ui_hub_fraglimit;
vmCvar_t	ui_hub_timelimit;
...
//PKMOD - Ergodic 09/17/2002 - add new medals to the UI for PKA
vmCvar_t	ui_scorePainkiller;
vmCvar_t	ui_scoreFrags;
...
//PKMOD - Ergodic 02/04/04 - add ui version of the PKA version for diplay on main splash screen 
vmCvar_t	ui_pkaVersion;
//PKMOD - Ergodic 02/05/04 - store high score for display on SP Postgame menu - endofgame
vmCvar_t	ui_oldHighScore;
...
static cvarTable_t		cvarTable[] = {
	...
	//PKMOD - Ergodic 11/07/00 - enable setting of hub limits - hub fraglimit
	{ &ui_hub_fraglimit, "ui_hub_fraglimit", "13", CVAR_ARCHIVE },
	{ &ui_hub_timelimit, "ui_hub_timelimit", "17", CVAR_ARCHIVE },
	...
	//PKMOD - Ergodic 12/07/02 - Set Team Names to Q3A standard
	{ &ui_redteam, "ui_redteam", "Red", CVAR_ARCHIVE },
	{ &ui_blueteam, "ui_blueteam", "Blue", CVAR_ARCHIVE },

	{ &ui_dedicated, "ui_dedicated", "0", CVAR_ARCHIVE },
	//PKMOD - Ergodic 01/04/04 - Default cvar to 0 instead of 3
	//		This cvar being 0 will break the PLAYER SETUP menu
	...
	//PKMOD - Ergodic 09/17/2002 - add new medals to the UI for PKA
	{ &ui_scorePainkiller, "ui_scorePainkiller", "0", CVAR_ARCHIVE},
	{ &ui_scoreFrags, "ui_scoreFrags", "0", CVAR_ARCHIVE},

//PKMOD - Ergodic 12/23/03 - Add functionality to set HUB Alternate maps in the UI
	{ &ui_hubalt1, "ui_hubalt1", "0", CVAR_ARCHIVE },
	{ &ui_hubalt2, "ui_hubalt2", "0", CVAR_ARCHIVE },
	{ &ui_hubalt3, "ui_hubalt3", "0", CVAR_ARCHIVE },
	{ &ui_hubalt4, "ui_hubalt4", "0", CVAR_ARCHIVE },
...
//PKMOD - Ergodic 02/02/04 - rename ui_scoreShutoutBonus to ui_scoreMedalBonus 
	{ &ui_scoreMedalBonus, "ui_scoreMedalBonus", "0", CVAR_ARCHIVE},
	...
	//PKMOD - Ergodic 01/04/04 - Default cvar to 1 instead of 0
	//		This cvar being 0 will break the PLAYER SETUP menu
	{ &ui_Q3Model, "ui_q3model", "1", CVAR_ARCHIVE},
	...
	//PKMOD - Ergodic 02/04/04 - add ui version of the PKA version for diplay on main splash screen 
	{ &ui_pkaVersion, "ui_pkaVersion", PKARENA_VERSION, CVAR_ARCHIVE},
	//PKMOD - Ergodic 02/05/04 - store high score for display on SP Postgame menu - endofgame
	{ &ui_oldHighScore, "ui_oldHighScore", "0", CVAR_ARCHIVE}
};
