...
#define	UI_TIMER_WEAPON_DELAY	50
//PKMOD changed from 250 to 50
...
tryagain:
	...
	//PKMOD - Ergodic 07/05/00 change the PKA gravity well weapon to the "hold" model
	//PKMOD - Ergodic 03/17/01 new PKA gravity well weapon to the "hold" model
	case WP_GRAVITY:
		pi->weaponModel = trap_R_RegisterModel( "models/weapons2/gwell/gwphold.md3" );
		MAKERGB( pi->flashDlightColor, 0.6f, 0.6f, 1.0f );
		break;

	//PKMOD - Ergodic 09/22/00 set the PKA autosentry to the "hold" model
	//PKMOD - Ergodic 12/17/00 update autosentry to the new model
	case WP_SENTRY:
		pi->weaponModel = trap_R_RegisterModel( "models/weapons2/autosentry/autosentry_hold.md3" );
		MAKERGB( pi->flashDlightColor, 0.6f, 0.6f, 1.0f );
		break;

	//PKMOD - Ergodic 07/05/00 change the PKA beartrap weapon to the "hold" model
	case WP_BEARTRAP:
		pi->weaponModel = trap_R_RegisterModel( "models/weapons2/beartrap/bearhold.md3" );
		//PKMOD - Ergodic 12/17/00 - force the "f"
		MAKERGB( pi->flashDlightColor, 0.6f, 0.6f, 1.0f );
		break;

	//PKMOD - Ergodic 07/05/00 change the PKA can of pork-n-beans weapon to the "hold" model
	case WP_BEANS:
		pi->weaponModel = trap_R_RegisterModel( "models/weapons2/beans/beanshold.md3" );
		MAKERGB( pi->flashDlightColor, 0.6f, 0.6f, 1.0f );
		break;
...
qboolean UI_RegisterClientModelname( playerInfo_t *pi, const char *modelSkinName, const char *headModelSkinName, const char *teamName ) {
	...
	//PKMOD - Ergodic 02/06/04 - debug failure to load ranger blue skin (inactive)
	//Com_Printf( "UI_RegisterClientModelname kin file: modelName>%s<, skinName>%s<, headModelName>%s<, headSkinName>%s<, teamName>%s<\n", modelName, skinName, headModelName, headSkinName, teamName );
	...
	if ( !UI_RegisterClientSkin( pi, modelName, skinName, headModelName, headSkinName, teamName) ) {
		if ( !UI_RegisterClientSkin( pi, modelName, "default", headModelName, "default", teamName ) ) {
			Com_Printf( "(ui1) Failed to load skin file: %s : %s\n", modelName, skinName );
			return qfalse;
		}
	}
